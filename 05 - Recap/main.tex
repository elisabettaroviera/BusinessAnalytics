...
\end{aligned}
\]
\hspace*{1.5em}Si definisce la \textcolor{RoyalBlue}{\textbf{funzione valore}}
\[
...
L’equazione precedente definisce una \textbf{funzione valore} \(V(\cdot):N\to\mathbb{R}\) che associa a ciascuno stato il suo valore ottimo.
...
\\ \hspace*{1.5em}
È già noto che tale approccio \textbf{\textcolor{DarkRed}{greedy}} non è atteso funzionare bene in generale: occorre bilanciare obiettivi di breve e lungo periodo introducendo una \textbf{funzione valore} \(V_t(\cdot)\).
\\ \hspace*{1.5em}
L’\textbf{idea fondamentale} della programmazione dinamica è che si può trovare una funzione valore tale da ottenere la prestazione ottima. Il valore \(V_t(s)\) dovrebbe essere il costo/ricavo atteso ottenuto applicando una politica ottima dal tempo \(t\) in avanti, partendo dallo stato \(s\).
...
ad esempio ottenuta da un’approssimazione della funzione valore ottima.
...
L’\textbf{equazione di Bellman} è un’\emph{equazione di ottimalità} e richiede di determinare la funzione valore per ogni
...
\\ \hspace*{1.5em}Risolvendo il problema precedente per ogni stato \(s_{T-1}\) si costruisce la funzione valore \(V_{T-1}(\cdot)\). Poi, svolgendo la ricorsione all’indietro nel tempo, si ottengono \(V_{T-2}(s_{T-2})\) e così via, fino a \(V_1(s_1)\).
...
\\ \hspace*{1.5em}
In un contesto \textbf{stocastico}, si può eseguire una simulazione \textbf{\emph{Monte Carlo}} come segue. Dato lo stato iniziale \(s_0\) e la funzione valore \(V_1(\cdot)\), si risolve il problema del primo stadio e si trova \(x_0^\star\); quindi si campiona \(\xi_1\) e si genera lo stato successivo \(s_1=g_1(s_0,x_0^\star,\xi_1)\). Ripetendo lo stesso ragionamento, dato lo stato \(s_t\) e la funzione valore \(V_{t+1}(\cdot)\), si risolve il problema del tempo \(t\) ottenendo \(x_t^\star\), e si prosegue fino a generare l’ultima decisione \(x_{T-1}^\star\) e lo stato terminale \(s_T\).
...
\\ \hspace*{1.5em}
Ora la funzione valore è definita come \textbf{\textcolor{DarkRed}{punto fisso}} di un operatore potenzialmente complicato,
...
\paragraph{Recap}
\textcolor{DarkGoldenrod}{\textbf{Allocazione discreta di risorse: il problema dello zaino}}
Il problema dello \textbf{zaino} rappresenta un caso paradigmatico di allocazione discreta di risorse, in cui la selezione delle attività è una decisione \textbf{tutto-o-niente} soggetta a un vincolo di capacità. La dipendenza dalle decisioni passate è catturata interamente dal \textbf{budget residuo}, che costituisce la variabile di stato naturale.  
\\ \hspace*{1.5em}
\textcolor{DarkGoldenrod}{\textbf{Riformulazione sequenziale e funzione valore}}
Introducendo uno stadio fittizio associato agli oggetti, il problema viene riformulato come allocazione sequenziale. La funzione valore consente di rappresentare il profitto ottimale ottenibile dai sottoproblemi residui e conduce a una ricorsione di Bellman semplice e completamente tabulabile nel caso discreto.  
\\ \hspace*{1.5em}
\textcolor{DarkGoldenrod}{\textbf{Implementazione numerica}}
La soluzione DP viene implementata tramite tabelle di valore e di decisione, calcolate con una procedura backward e utilizzate per ricostruire la soluzione ottima in avanti. La complessità risultante è \textbf{pseudo-polinomiale} e cresce linearmente con il budget.  
\\ \hspace*{1.5em}
\textcolor{DarkGoldenrod}{\textbf{Allocazione continua del budget}}
Nel caso continuo, il budget viene allocato tramite decisioni reali e funzioni di profitto concave. In presenza di soluzioni interne, le condizioni di ottimalità sono caratterizzate dalla Lagrangiana, ma lo stesso problema può essere riformulato in termini di programmazione dinamica.  
\\ \hspace*{1.5em}
\textcolor{DarkGoldenrod}{\textbf{Approssimazione e interpolazione}}
Nel continuo la funzione valore è definita su uno spazio \textbf{infinito-dimensionale}. La risoluzione numerica richiede discretizzazione dello stato e tecniche di interpolazione. Le spline cubiche permettono di evitare le oscillazioni tipiche dell’interpolazione polinomiale globale.  
\\ \hspace*{1.5em}
\textcolor{DarkGoldenrod}{\textbf{Controllo stocastico delle scorte}}
La programmazione dinamica viene applicata a problemi di lot-sizing con domanda aleatoria discreta. La ricorsione DP incorpora l’aspettativa rispetto alla domanda futura e conduce a politiche ottime tabellari.  
\\ \hspace*{1.5em}
\textcolor{DarkGoldenrod}{\textbf{Struttura e politiche ottime}}
Nel caso deterministico emergono forti proprietà strutturali, come il risultato di Wagner--Whitin, che consentono riformulazioni efficienti come cammini minimi. In ambiente stocastico compaiono politiche strutturate di tipo \textbf{base-stock} e \textbf{\((s,S)\)}.  
\\ \hspace*{1.5em}
\textcolor{DarkGoldenrod}{\textbf{Limiti della DP}}
L’approccio DP soffre di limitazioni legate alla dimensionalità dello stato, alla difficoltà dei sottoproblemi, al calcolo delle aspettative e alla complessità della modellazione.
...
\paragraph{Funzione valore e ricorsione di Bellman}
...
\textbf{Se} il suo peso \(w_k\) non è compatibile con il budget residuo \(s\), cioè se \textbf{\(s < w_k\)}, l’oggetto non può essere selezionato. In questo caso la variabile di stato rimane invariata e la funzione valore nello stato \(s\) coincide con quella dello stadio successivo, ossia \(V_k(s)=V_{k+1}(s)\).  
\\ \hspace*{1.5em}
...
Per l’ultimo oggetto, corrispondente allo stadio \(k=n\), la funzione valore è definita come
\[
V_n(s)=
\begin{cases}
0 & \text{se } 0 \le s < w_n,\\
...
La funzione \textcolor{DarkGoldenrod}{\texttt{DPKnapsack}} memorizza la funzione valore in una tabella indicizzata per stato e stadio e una tabella delle decisioni ottime.  
\\ \hspace*{1.5em}
...
Si considera una versione \textbf{continua} del problema di allocazione delle risorse, in cui un budget
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% PDF 9 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\vspace{-1cm}
\textcolor{DeepPink}{\section{CAP 9 -- Modelli della programmazione dinamica}}
