...
\paragraph{Recap} \textcolor{red}{MANCA!!}
...
\textcolor{red}{\textbf{ARRIVATA QUI MANCA DA RIVEDERE}}
...
Un modo naturale per introdurre \textbf{decisioni adattive in condizioni di incertezza} è l’utilizzo degli \textbf{alberi di decisione}. Gli alberi di decisione consentono di rappresentare in modo esplicito la sequenza temporale delle decisioni e delle realizzazioni aleatorie, evidenziando come le decisioni possano essere adattate sulla base delle informazioni che si rendono disponibili nel tempo.

\paragraph{Tipologie di nodi}
Un albero di decisione è costituito da due tipi fondamentali di nodi:
\begin{itemize}[label=-]
    \item \textbf{Nodi decisionali}, rappresentati da quadrati, che corrispondono a \textbf{scelte discrete tra alternative mutuamente esclusive}. In questi nodi il decisore deve selezionare una sola azione tra quelle disponibili.
    \item \textbf{Nodi casuali} (\emph{chance nodes}), rappresentati da cerchi, che descrivono la \textbf{realizzazione di esiti aleatori}. A ciascun esito $\omega$ è associata una probabilità $p_\omega$, e la somma delle probabilità degli esiti che partono da uno stesso nodo casuale è pari a 1.
\end{itemize}

\paragraph{Struttura di un albero di decisione}
Un albero di decisione è composto da una sequenza di nodi decisionali e nodi casuali, che in genere si \textbf{alternano}. L’albero termina con \textbf{nodi terminali}, rappresentati da punti, ai quali sono associati i \textbf{risultati finali}, tipicamente espressi in termini monetari.

\paragraph{Strategia e criterio di ottimalità}
Risolvere un problema rappresentato mediante un albero di decisione significa individuare una \textbf{strategia}, ovvero una regola che specifica quale decisione assumere in ciascun nodo decisionale che può essere visitato.  
Assumendo che i risultati abbiano natura monetaria, il criterio più naturale è la \textbf{massimizzazione del valore monetario atteso} (EMV, \emph{Expected Monetary Value}).

\paragraph{Valore monetario atteso e procedura backward}
Quando un nodo decisionale è seguito da nodi casuali, ciascun nodo casuale viene etichettato con il proprio \textbf{valore monetario atteso}. Questo consente di confrontare le alternative disponibili nel nodo decisionale.  
La procedura di soluzione procede \textbf{a ritroso nel tempo} (\emph{backward induction}), iniziando dai nodi terminali e risalendo progressivamente verso la radice dell’albero. Un nodo può essere valutato solo quando tutti i suoi successori sono già stati valutati.

\paragraph{Esempio: introduzione di un nuovo prodotto (idea generale)}
...
L’indagine fornisce informazione parziale che modifica le probabilità condizionate di successo, ma non la probabilità incondizionata complessiva. La soluzione ottimale si ottiene confrontando il valore monetario atteso delle diverse strategie, tenendo conto del costo dell’informazione e delle decisioni ottimali successive ai diversi esiti osservati.

\paragraph{Valore dell’informazione}
L’esempio mostra che gli alberi di decisione consentono di stimare il \textbf{valore dell’informazione}. In particolare, il valore dell’informazione parziale è dato dalla differenza tra il valore atteso ottenibile utilizzando l’informazione e il valore atteso ottenibile senza di essa, e rappresenta il \textbf{massimo importo che conviene spendere} per acquisire informazione aggiuntiva.
...
\[
5^{*} = \min_{x \in X} \mathbb{E}_{P}\!\left[ 5(x,\tilde{\xi}) \right].
\tag{9}
\]
In questo contesto la decisione viene presa \textbf{here-and-now}, prima della realizzazione dell’incertezza.

\paragraph{Ottimizzazione con informazione perfetta}
Ci si può chiedere come migliorerebbe il valore ottimo se fosse possibile \textbf{posticipare la decisione} e osservare prima lo scenario realizzato. Formalmente, ciò equivale a scambiare l’operatore di minimo con il valore atteso:
\[
5^{*}_{PI} = \mathbb{E}_{P}\!\left[ \min_{x \in X} 5(x,\tilde{\xi}) \right].
\tag{10}
\]
Il pedice $PI$ indica che questo valore corrisponde all’ottimo ottenibile in presenza di \textbf{informazione perfetta}.

\paragraph{Relazione tra i due valori}
...
La differenza tra i due valori ottimi definisce il \textbf{valore atteso dell’informazione perfetta}:
\[
EVPI = 5^{*} - 5^{*}_{PI}.
\tag{11}
\]
...
\paragraph{Esempio: scelta di una strategia di investimento (idea generale)}
...
Nel caso \textbf{here-and-now}, la decisione viene presa prima di osservare lo stato dell’economia e la strategia viene scelta massimizzando il rendimento atteso.  
Nel caso \textbf{wait-and-see}, la strategia ottimale può essere selezionata dopo aver osservato lo scenario realizzato, ottenendo un valore atteso maggiore. La differenza tra i due valori rappresenta l’EVPI.

\paragraph{Interpretazione dell’EVPI}
...
\[
5^{*}_{EV} = \min_{x \in X} 5\!\left(x, \mathbb{E}_{P}[\tilde{\xi}]\right),
\]
la cui soluzione è detta \textbf{expected-value solution} e viene indicata con $\bar{x}$.

\paragraph{Valutazione della soluzione a valore atteso}
La soluzione $\bar{x}$ deve essere valutata nel contesto stocastico originale. Il costo risultante è la variabile aleatoria $5(\bar{x},\tilde{\xi})$, di cui si considera il valore atteso:
\[
5_{EEV} = \mathbb{E}_{P}\!\left[ 5(\bar{x},\tilde{\xi}) \right].
\]
Il confronto corretto non è tra $5^{*}_{EV}$ e $5^{*}$, ma tra $5_{EEV}$ e $5^{*}$.

\paragraph{Valore della soluzione stocastica (VSS)}
Il \textbf{valore della soluzione stocastica} è definito come
\[
VSS = 5_{EEV} - 5^{*},
\]
nel caso di minimizzazione (con scambio dei termini nel caso di massimizzazione).  
È possibile dimostrare che $VSS \ge 0$. Quando il VSS è elevato, lo sforzo computazionale richiesto per risolvere il problema stocastico completo è giustificato dal miglioramento ottenuto rispetto alla soluzione deterministica a valore atteso.

\paragraph{Collegamento con modelli multi-stadio}
Il concetto di VSS viene ulteriormente chiarito attraverso esempi numerici e rappresenta un ponte naturale verso i \textbf{modelli di ottimizzazione a due stadi e multi-stadio}, nei quali le decisioni possono essere adattate dopo l’osservazione dell’incertezza.

\subsection{An introductory two-stage model: Assemble-to-order}

\paragraph{Contesto e motivazione}
Si consideri un esempio didattico di \textbf{pianificazione della produzione} in cui i prodotti finiti sono ottenuti assemblando un insieme di componenti. Ogni prodotto finale è caratterizzato da un insieme di caratteristiche, per ciascuna delle quali sono disponibili opzioni alternative che richiedono componenti diversi. Dalla combinazione di un numero limitato di componenti può derivare un numero molto elevato di prodotti finali.

Per questo motivo, non è possibile proteggersi dall’incertezza della domanda mantenendo scorte di prodotti finiti. D’altra parte, una strategia puramente \emph{make-to-order} può risultare poco efficace a causa dei lunghi tempi di approvvigionamento dei componenti. In un ambiente \textbf{Assemble-To-Order (ATO)}, in cui l’assemblaggio è rapido, è invece possibile mantenere a magazzino i componenti e assemblare i prodotti finali solo dopo aver osservato la domanda.

Questo contesto suggerisce naturalmente una \textbf{strategia a due stadi}:
\begin{itemize}[label=-]
    \item pianificazione della produzione dei componenti \textbf{here-and-now}, sotto incertezza sulla domanda;
    \item assemblaggio dei prodotti finali in modalità \textbf{wait-and-see}, dopo l’osservazione degli ordini.
\end{itemize}

\paragraph{Struttura del problema}
Si considerino tre prodotti finali $(\mu_1,\mu_2,\mu_3)$ ottenuti assemblando cinque componenti $(2_1,2_2,2_3,2_4,2_5)$. La distinta base è piatta, con due soli livelli (prodotti finali e componenti), ed è riportata in Tabella~2.  
Ad esempio, per assemblare una unità del prodotto $\mu_2$ sono necessari un componente $2_1$, uno $2_2$ e uno $2_4$. I componenti $2_1$ e $2_2$ sono comuni a più prodotti finali, mentre gli altri sono specifici.

La produzione dei componenti utilizza tre gruppi di risorse produttive $(r_1,r_2,r_3)$. La Tabella~3 riporta:
\begin{itemize}[label=-]
    \item i tempi di lavorazione dei componenti su ciascun gruppo di risorse;
    \item la capacità disponibile di ciascun gruppo;
    \item il costo unitario dei componenti.
\end{itemize}

Si assume che l’assemblaggio non costituisca un collo di bottiglia. Il costo totale dei componenti necessari per ciascun prodotto finale è pari a $20+30+10=60$.

\paragraph{Dati di mercato}
I dati di domanda e di prezzo sono riportati in Tabella~4. La domanda è modellata tramite tre scenari equiprobabili $(B_1,B_2,B_3)$. Il prezzo di vendita è sempre superiore al costo dei componenti, ma il prodotto $\mu_3$ risulta il più redditizio, mentre $\mu_2$ è il meno profittevole.

\paragraph{Modello deterministico a valore atteso}
Ignorando l’incertezza della domanda e sostituendola con il valore medio, si introducono le seguenti variabili decisionali:
\begin{itemize}[label=-]
    \item $G_s \in \mathbb{Z}_+$, $s=1,\dots,5$, numero di componenti prodotti;
    \item $H_j \in \mathbb{Z}_+$, $j=1,\dots,3$, numero di prodotti finali assemblati.
\end{itemize}

Il problema di massimizzazione del profitto può essere formulato come:
\[
\max \; - \sum_{s=1}^{5} c_s G_s + \sum_{j=1}^{3} p_j H_j
\tag{12}
\]
soggetto a:
\[
\sum_{s=1}^{5} a_{s\ell} G_s \le C_\ell, \quad \ell = 1,2,3,
\tag{13}
\]
\[
H_j \le d_j, \quad j=1,2,3,
\tag{14}
\]
\[
\sum_{j=1}^{3} b_{sj} H_j \le G_s, \quad s=1,\dots,5,
\tag{15}
\]
\[
G_s, H_j \in \mathbb{Z}_+.
\]

La soluzione ottima produce principalmente il prodotto più profittevole $\mu_3$, seguita da una quantità limitata di $\mu_1$, mentre $\mu_2$ viene escluso. Il valore ottimo del problema deterministico risulta $5_{EV}=3220$.
...
\begin{itemize}[label=-]
    \item una domanda scenario-dipendente $d^B_j$ con probabilità $p_B$;
    \item variabili di secondo stadio $H^B_j \in \mathbb{Z}_+$ per l’assemblaggio adattivo.
\end{itemize}

Il modello stocastico a due stadi è:
\[
\max \; - \sum_{s=1}^{5} c_s G_s
+ \sum_{B=1}^{3} p_B
\left(
\sum_{j=1}^{3} p_j H^B_j
\right)
\tag{16}
\]
soggetto a:
...
\]
\[
H^B_j \le d^B_j, \quad j=1,2,3,\; B=1,2,3,
\tag{18}
\]
\[
\sum_{j=1}^{3} b_{sj} H^B_j \le G_s, \quad s=1,\dots,5,\; B=1,2,3,
\tag{19}
\]
\[
G_s, H^B_j \in \mathbb{Z}_+.
\]

\paragraph{Soluzione del modello stocastico}
La soluzione stocastica produce quantità di componenti meno estreme e decisioni di assemblaggio che variano a seconda dello scenario. Il valore atteso del profitto risulta
\[
5^{*} = 2883.33.
\]
...
Il confronto diretto tra $5^{*}$ e $5_{EV}$ non è significativo. Occorre invece fissare le decisioni di primo stadio e risolvere, per ciascuno scenario $B$, il problema di secondo stadio:
\[
\varphi^B(x^\circ) =
\max \sum_{j=1}^{3} p_j H^B_j
\]
soggetto a:
\[
H^B_j \le d^B_j,
\quad
\sum_{j=1}^{3} b_{sj} H^B_j \le G^\circ_s,
\quad
H^B_j \in \mathbb{Z}_+.
\]

\paragraph{Valutazione della soluzione deterministica}
Applicando questa procedura alla soluzione deterministica, si osserva che uno degli scenari conduce a un grave spreco di componenti e a un profitto molto inferiore rispetto a quello previsto. Il valore atteso effettivo risulta:
\[
5_{EEV} = 2320,
\]
ben al di sotto di $5_{EV}=3220$.

\paragraph{Valore della soluzione stocastica}
Nel caso in esame:
\[
VSS = 5^{*} - 5_{EEV}
= 2883.33 - 2320
= 563.33.
\]
Questo valore quantifica il beneficio derivante dall’uso di un modello stocastico rispetto a una pianificazione deterministica basata su valori medi.

\subsection{Two-stage models}

\paragraph{Dall’ATO ai modelli a due stadi con ricorso}
Il modello \emph{assemble-to-order} della sezione precedente fornisce una buona introduzione ai \textbf{modelli di programmazione lineare stocastica a due stadi con ricorso} (\emph{recourse}), che rappresentano l’esempio più semplice di modelli di ottimizzazione adattivi.

\paragraph{Decisioni di primo e secondo stadio}
...
\end{itemize}
Le decisioni di secondo stadio sono anche dette \textbf{decisioni di ricorso}. Si osserva immediatamente che tali decisioni sono in realtà \textbf{politiche decisionali}, cioè funzioni che mappano la realizzazione dei fattori di rischio in una decisione ammissibile; pertanto appartengono a uno spazio \textbf{infinito-dimensionale}.

\paragraph{Formulazione annidata del modello}
Il modello può essere formulato come due problemi di ottimizzazione annidati:
\[
\boxed{
\begin{aligned}
\min_{x}\;& c^{T}x + Q(x) \\
...
\[
\boxed{
Q(x) = \mathbb{E}_{P}\!\left[\varphi(x,\tilde{\xi})\right].
\tag{22}}
\]
Il problema di secondo stadio, dato $x$ e una realizzazione $\xi$ della variabile aleatoria $\tilde{\xi}$, è:
\[
\boxed{
\begin{aligned}
\varphi(x,\xi) = \min_{y}\;& q(\xi)^{T}y \\
...
Nel problema di secondo stadio sia la decisione di primo stadio $x$ sia la realizzazione $\xi$ dei fattori di rischio sono dati. Risolvendo il secondo stadio per ogni realizzazione $\xi$, si definisce implicitamente una funzione $y(\tilde{\xi})$, mostrando che le variabili di secondo stadio sono effettivamente funzioni. In questa formulazione la matrice di ricorso $W$ non dipende da variabili aleatorie e si parla di \textbf{ricorso fisso} (\emph{fixed recourse}). La formulazione evidenzia che la programmazione lineare stocastica con ricorso è, in generale, un \textbf{problema di programmazione non lineare} (poiché $Q(x)$ dipende da $x$ tramite un problema di ottimizzazione interno).

\paragraph{Difficoltà computazionali della funzione di ricorso}
...
\paragraph{Discretizzazione tramite scenari}
Poiché le variabili di secondo stadio sono funzioni in uno spazio infinito-dimensionale, una strategia comune consiste nella discretizzazione tramite campionamento. Si genera un \textbf{albero di scenario} (un \emph{fan}) in cui l’evento $\omega^{B}$ corrisponde alla realizzazione dello scenario $B\in S$, con $S$ insieme degli scenari. Se gli scenari sono campionati via Monte Carlo semplice, le probabilità sono uniformi: $p_{B}=1/|S|$ (sono possibili anche metodi più sofisticati di generazione scenari).

\paragraph{Modello deterministico equivalente (LP a grande scala)}
La discretizzazione conduce al seguente modello:
\[
\boxed{
\begin{aligned}
\min\;& c^{T}x + \sum_{B\in S} p_{B}\, (q^{B})^{T}y^{B}\\
\text{s.t. }\;& Ax=b\\
& Wy^{B} + T^{B}x = h^{B}, \quad B\in S \\
& x,\; y^{B}\ge 0.
\end{aligned}}
\]
Questo è un \textbf{LP} (sebbene potenzialmente di grande dimensione).

\paragraph{Fattibilità del secondo stadio e concetti di ricorso completo}
Una questione centrale è stabilire se il secondo stadio sia fattibile per ogni scelta delle variabili di primo stadio e per ogni realizzazione delle variabili aleatorie. Occorre restringere l’insieme delle decisioni \emph{here-and-now} al dominio in cui il secondo stadio è fattibile (equivalentemente, la funzione di ricorso è limitata; come usuale, a un problema infeattibile si associa costo infinito).

Riscrivendo i vincoli di collegamento come
\[
Wy^{B}=h^{B}-T^{B}x, \quad B\in S,
\]
il secondo stadio è fattibile quando il termine noto può essere espresso come combinazione conica delle colonne di $W$. In tal caso si parla di \textbf{complete recourse}. Se il ricorso completo non vale per decisioni arbitrarie di primo stadio, ma vale per le decisioni che soddisfano i vincoli del primo stadio, si parla di \textbf{relatively complete recourse}.

\paragraph{Penalità e modelli elastici}
Nei problemi di business è spesso possibile introdurre flessibilità tramite penalità, in modo da evitare infeattibilità al secondo stadio. Nelle sezioni successive l’idea viene illustrata con esempi concreti.

\subsection{The plant location model}

\paragraph{Descrizione del problema e rete bipartita}
Il classico \textbf{plant location model} è il più semplice problema di \textbf{network design}. Si considera una rete bipartita con un insieme $P$ di nodi sorgente potenziali e un insieme $D$ di nodi domanda. I nodi sorgente rappresentano impianti produttivi che possono essere aperti oppure no; i nodi domanda possono rappresentare magazzini regionali o centri retail. In ciascun nodo domanda si realizza una domanda aleatoria $\delta_{j}(\omega)$, $j\in D$, che va soddisfatta al costo minimo.

\paragraph{Dati del modello}
Sono necessari i seguenti dati:
\begin{itemize}[label=-]
    \item per ogni $i\in P$, un costo fisso di apertura $f_i$ e una capacità $u_i$;
    \item per ogni $j\in D$ e scenario $B\in S$, una domanda $d^{B}_{j}$ (discretizzazione della domanda aleatoria);
    \item per ogni arco $(i,j)$, un costo unitario di trasporto $c_{ij}$ (costi variabili lineari).
\end{itemize}

\paragraph{Struttura a due stadi e variabili}
...
x^{B}_{ij}\ge 0,
\]
dove $x^{B}_{ij}$ è il flusso da $i\in P$ a $j\in D$ nello scenario $B\in S$.
Le variabili di localizzazione $H_i$ sono \textbf{di primo stadio} (design), mentre i flussi $x^{B}_{ij}$ sono \textbf{di secondo stadio} (controllo).

\paragraph{Formulazione MILP a due stadi con ricorso}
Si ottiene il seguente modello MILP stocastico:
\[
\boxed{
\begin{aligned}
\min\;& \sum_{i\in P} f_i H_i + \sum_{B\in S} p_B \left(\sum_{i\in P}\sum_{j\in D} c_{ij} x^{B}_{ij}\right)\\
\text{s.t. }\;& \sum_{i\in P} x^{B}_{ij} = d^{B}_{j}, \quad B\in S,\; j\in D\\
& \sum_{j\in D} x^{B}_{ij} \le u_i H_i, \quad B\in S,\; i\in P\\
& x^{B}_{ij}\ge 0,\quad H_i\in\{0,1\}.
\end{aligned}}
\]

\paragraph{Scenari estremi e necessità di formulazioni elastiche}
È opportuno chiedersi cosa accade se viene incluso uno scenario estremo con domanda molto elevata. Potrebbe non essere nemmeno possibile soddisfare tale domanda; inoltre, la pianificazione della capacità verrebbe guidata da scenari estremi ma molto improbabili, producendo soluzioni eccessivamente costose. In questi casi conviene ricorrere a formulazioni elastiche, permettendo violazioni penalizzate dei vincoli.

\paragraph{Modello elastico con domanda non soddisfatta penalizzata}
Sia $s^{B}_{j}\ge 0$ la quantità di domanda non soddisfatta nel nodo $j$ nello scenario $B$. Tali variabili entrano nell’obiettivo moltiplicate per un coefficiente di penalità $\pi_j$. Il modello diventa:
\[
\boxed{
\begin{aligned}
\min\;& \sum_{i\in P} f_i H_i
+ \sum_{B\in S} p_B \left(\sum_{i\in P}\sum_{j\in D} c_{ij} x^{B}_{ij}\right)
+ \sum_{B\in S} p_B \left(\sum_{j\in D} \pi_j s^{B}_{j}\right)\\
\text{s.t. }\;& \sum_{i\in P} x^{B}_{ij} + s^{B}_{j} = d^{B}_{j}, \quad B\in S,\; j\in D\\
& \sum_{j\in D} x^{B}_{ij} \le u_i H_i, \quad B\in S,\; i\in P\\
& x^{B}_{ij}\ge 0,\; s^{B}_{j}\ge 0,\quad H_i\in\{0,1\}.
\end{aligned}}
\]
La quantificazione delle penalità $\pi_j$ dipende dal significato di $s^{B}_{j}$: può rappresentare domanda effettivamente non servita (con penalità differenziate per priorità di mercato) oppure domanda soddisfatta tramite fornitori d’emergenza (con penalità pari al costo effettivo di tale opzione).
...
\subsection{A two-stage newsvendor model}

\paragraph{Motivazione e struttura informativa}
Si consideri una versione a due stadi e multi-item del \textbf{newsvendor problem}, soggetta a vincoli di capacità. Il modello è motivato da un produttore di sport fashion che deve avviare la produzione in anticipo sotto forte incertezza (per vincoli di capacità), ma può aggiornare previsioni e piani dopo aver osservato una prima parte delle vendite ai retailer. La struttura informativa può essere rappresentata da un albero a tre livelli: al nodo radice si decide il primo lotto; nei nodi intermedi si decide un secondo lotto sotto incertezza ridotta; nei nodi foglia si osservano le vendite effettive e si contabilizzano costi di overage/underage.

\paragraph{Nodi e predecessori}
Ogni nodo foglia $\eta\in N_2$ ha esattamente un predecessore (antecedente) $\alpha(\eta)\in N_1$. Il nodo radice $0$ è l’antecedente diretto di ogni nodo in $N_1$. Le decisioni dell’ultimo livello sono in realtà variabili fittizie di contabilizzazione (surplus/shortfall penalizzati), non variabili di controllo o design.

\paragraph{Set e dati}
Sono necessari i seguenti insiemi e parametri:
\begin{itemize}[label=-]
    \item $I$: insieme degli item (SKU).
    \item $C_1$ e $C_2$: capacità massime (numero massimo di item producibili) nel primo e nel secondo run produttivo.
    ...
    \item domanda $d^{\eta}_{i}$ per ciascun $i\in I$ e foglia $\eta\in N_2$, con probabilità associata $p^{\eta}$.
    \item coefficienti di penalità di overage $c^{+}_{i}$ e underage $c^{-}_{i}$ per ciascun $i\in I$.
\end{itemize}
...
\begin{itemize}[label=-]
    ...
    \item variabili di contabilizzazione $u^{\eta}_{i}$ (underage) e $v^{\eta}_{i}$ (overage) ai nodi foglia $\eta\in N_2$.
\end{itemize}

\paragraph{Formulazione (stochastic MILP)}
La formulazione è:
\[
\boxed{
\begin{aligned}
\min\;& \sum_{\eta\in N_2} p^{\eta}\left[\sum_{i\in I}\big(c^{+}_{i} v^{\eta}_{i} + c^{-}_{i} u^{\eta}_{i}\big)\right] \\
\text{s.t. }\;& \sum_{i\in I} x^{0}_{i} \le C_1 \\
& x^{0}_{i} \ge L_i z^{0}_{i}, \quad x^{0}_{i} \le C_1 z^{0}_{i}, \quad i\in I \\
& \sum_{i\in I} x^{n}_{i} \le C_2,\quad n\in N_1 \\
& x^{n}_{i} \ge L_i z^{n}_{i}, \quad x^{n}_{i} \le C_2 z^{n}_{i}, \quad i\in I,\; n\in N_1 \\
& x^{0}_{i} + x^{\alpha(\eta)}_{i} = d^{\eta}_{i} + v^{\eta}_{i} - u^{\eta}_{i},
\quad i\in I,\; \eta\in N_2 \\
& x^{n}_{i}\in\mathbb{Z}_+,\; z^{n}_{i}\in\{0,1\},\quad i\in I,\; n\in\{0\}\cup N_1\\
& u^{\eta}_{i}\ge 0,\; v^{\eta}_{i}\ge 0,\quad i\in I,\; \eta\in N_2.
\end{aligned}}
\]

\paragraph{Interpretazione dei vincoli}
La funzione obiettivo (26) è il costo atteso di deviazione. I vincoli (27) e (29) impongono la capacità dei due run produttivi. I vincoli (28) e (30) sono i vincoli \emph{big-M} che collegano variabili continue/intere e binarie: se $z=1$ la produzione è tra lotto minimo e capacità; se $z=0$ la produzione viene forzata a 0. Il vincolo (31) misura la deviazione tra produzione totale e domanda osservata. Non è necessario imporre integrità sulle variabili di deviazione, poiché segue dall’integrità delle variabili di produzione.

\subsection{Multistage stochastic linear programming with recourse}

\paragraph{Generalizzazione ai modelli multistadio}
Le formulazioni multistadio sorgono come generalizzazione dei modelli a due stadi. Concettualmente, si annidano funzioni di ricorso corrispondenti ai diversi stadi decisionali. In questo caso, le decisioni future possono dipendere dall’intera storia del processo stocastico dei fattori di rischio. Si introduce la notazione:
\[
\tilde{\xi}^{(t)} = (\tilde{\xi}_1,\tilde{\xi}_2,\dots,\tilde{\xi}_t),
\qquad
\tilde{\xi}^{(1)}=\tilde{\xi}_1.
\]
La decisione \emph{here-and-now} è $x_0\in X_0$ e si risolve:
\[
\min_{x_0\in X_0}\left\{ f_0(x_0) + \mathbb{E}\big[\varphi_1(x_0,\tilde{\xi}_1)\big]\right\}.
\]
Il termine $\varphi_1(x_0,\xi_1)$ è definito ricorsivamente come:
\[
\varphi_1(x_0,\xi_1)=\min_{x_1\in X_1(x_0,\xi_1)}
\left\{ f_1(x_1,\xi_1) + \mathbb{E}\!\left[\varphi_2\!\big(x_1,\tilde{\xi}^{(2)}\big)\mid \tilde{\xi}_1=\xi_1\right]\right\}.
\]
Per $t=2,\dots,T$:
\[
\boxed{
\begin{aligned}
\varphi_t(x_{t-1},\xi^{(t)})=
\min_{x_t\in X_t(x_{t-1},\xi^{(t)})}
\Big\{
f_t(x_t,\xi^{(t)}) \\
\qquad\qquad\qquad
+ \mathbb{E}\!\left[\varphi_{t+1}\!\big(x_t,\tilde{\xi}^{(t+1)}\big)\mid \tilde{\xi}^{(t)}=\xi^{(t)}\right]
\Big\}.
\end{aligned}}
\tag{32}
\]
All’ultimo stadio, per $t=T$, si può porre $\varphi_{T+1}(\cdot)=0$.

\paragraph{Funzione di ricorso e riscrittura compatta}
Definendo la funzione di ricorso
\[
Q_{t+1}(x_t,\xi^{(t)})=
\mathbb{E}\!\left[\varphi_{t+1}\!\big(x_t,\tilde{\xi}^{(t+1)}\big)\mid \tilde{\xi}^{(t)}=\xi^{(t)}\right],
\]
si può riscrivere (32) come:
\[
\boxed{
\varphi_t(x_{t-1},\xi^{(t)})=
\min_{x_t\in X_t(x_{t-1},\xi^{(t)})}
\left\{ f_t(x_t,\xi^{(t)}) + Q_{t+1}(x_t,\xi^{(t)})\right\}.}
\tag{33}
\]

\paragraph{Formulazione funzionale e non-anticipatività}
Un’alternativa è enfatizzare che le decisioni di ricorso sono funzioni non-anticipative del cammino osservato:
$x_t(\tilde{\xi}^{(t)})$. Si ottiene la formulazione funzionale:
\[
\boxed{
\begin{aligned}
\min_{x_0,\;x_1(\cdot),\dots,x_T(\cdot)}\;
\mathbb{E}\Big[
f_0(x_0)
+ f_1\big(x_1(\tilde{\xi}^{(1)}),\tilde{\xi}_1\big)
+ \cdots \\
\qquad\qquad\qquad\qquad\qquad
+ f_T\big(x_T(\tilde{\xi}^{(T)}),\tilde{\xi}_T\big)
\Big]\\
\text{s.t. } x_0\in X_0,\\
x_t(\xi^{(t)})\in X_t\big(x_{t-1}(\xi^{(t-1)}),\xi^{(t)}\big),
\quad t=1,\dots,T.
\end{aligned}}
\]
Questa formulazione appare più chiara, ma la scelta dipende dalla strategia di soluzione: la formulazione annidata si presta a decomposizione ricorsiva, mentre quella funzionale suggerisce la restrizione delle politiche a un sottospazio finito-dimensionale di politiche semplici.

\paragraph{Alberi di scenario multistadio}
Un approccio semplice è basato su alberi di scenario multistadio, che discretizzano il processo stocastico. Ogni scenario $\omega$ corrisponde a un cammino:
\[
\xi^{(T)}(\omega)=\big(\xi_1(\omega),\xi_2(\omega),\dots,\xi_T(\omega)\big).
\]
A ciascun nodo $n$ dell’albero (insieme dei nodi $N$) è associato un vettore di variabili $x_n$. Ogni nodo ha un unico antecedente $\alpha(n)$ (eccetto la radice), e un insieme di successori diretti; le probabilità condizionate lungo gli archi rappresentano la struttura informativa.

\paragraph{Non-anticipatività nel tree}
La struttura ad albero è fondamentale per rappresentare la non-anticipatività: scenari che condividono gli stessi nodi fino a un certo tempo non sono distinguibili in quel periodo, e quindi devono condividere le stesse decisioni nei nodi comuni. Solo quando gli scenari divergono in nodi differenti è possibile adottare decisioni differenti.

\subsection{A multistage financial planning model with proportional transaction costs}

\paragraph{Asset--liability management su albero di scenario}
Si formula un modello di \textbf{asset--liability management} in cui si scambia un insieme di strumenti finanziari (asset) per far fronte a un flusso di passività (liabilities). L’incertezza è rappresentata mediante un albero di scenario.
\begin{itemize}[label=-]
    ...
    \item Si considerano costi di transazione proporzionali (lineari): per acquistare o vendere si paga una percentuale $\tau$ del valore scambiato, sia in acquisto sia in vendita.
\end{itemize}

La radice è il nodo $0$, in cui si effettua la prima allocazione considerando le dotazioni iniziali $\bar{h}^{0}_i$. L’insieme delle foglie è $S$; i nodi di trading intermedi sono
\[
T = N \setminus \big(\{0\}\cup S\big).
\]
...
Si introducono:
\begin{itemize}[label=-]
    \item $b^{n}_i$: quantità di asset $i$ acquistata nel nodo $n$;
    \item $s^{n}_i$: quantità di asset $i$ venduta nel nodo $n$;
    \item $h^{n}_i$: quantità detenuta di asset $i$ nel nodo $n$ dopo il ribilanciamento;
    \item $W^{\ell}$: ricchezza terminale nella foglia $\ell\in S$.
\end{itemize}
Le variabili $b^{n}_i$, $s^{n}_i$, $h^{n}_i$ sono definite per $n\in N\setminus S$ (nessun ribilanciamento alle foglie). Sia $U(\cdot)$ l’utilità non lineare della ricchezza.

\paragraph{Modello}
\[
\boxed{
\begin{aligned}
\max\;& \sum_{\ell\in S} p_{\ell}\, U\!\left(W^{\ell}\right) \\
\text{s.t. }\;& h^{0}_i = \bar{h}^{0}_i + b^{0}_i - s^{0}_i, \quad i\in I \\
& h^{n}_i = h^{\alpha(n)}_i + b^{n}_i - s^{n}_i, \quad i\in I,\; n\in T \\
& (1-\tau)\sum_{i\in I} P^{n}_i s^{n}_i - (1+\tau)\sum_{i\in I} P^{n}_i b^{n}_i = L_n,
\quad n\in N\setminus S \\
& W^{\ell} = (1-\tau)\sum_{i\in I} P^{\ell}_i\, h^{\alpha(\ell)}_i - L_{\ell},
\quad \ell\in S\\
& h^{n}_i,\; b^{n}_i,\; s^{n}_i,\; W^{\ell} \ge 0.
\end{aligned}}
\]

\paragraph{Interpretazione dei vincoli}
L’obiettivo (34) è l’utilità attesa della ricchezza terminale. Il vincolo (35) impone il bilancio iniziale sugli asset, tenendo conto delle posizioni iniziali. Il vincolo (36) descrive l’evoluzione delle detenzioni nei nodi di trading. Il vincolo (37) garantisce che la cassa generata vendendo (al netto dei costi di transazione) e l’eventuale reinvestimento in acquisti (inclusi i costi) consenta di soddisfare la liability del nodo. Il vincolo (38) valuta la ricchezza terminale alle foglie, dopo liquidazione del portafoglio e pagamento dell’ultima liability.

\paragraph{Osservazioni operative}
In pratica il modello verrebbe risolto ripetutamente con una logica \emph{rolling horizon}; il ruolo dell’utilità terminale è evitare effetti distorsivi di fine orizzonte, imponendo che il portafoglio sia in una buona posizione alla fine dell’orizzonte di pianificazione. In un modello più realistico si dovrebbero includere eventuali contributi in ingresso e shortfall penalizzati rispetto alle liabilities.

\subsection{Multiperiod vs. multistage models}

\paragraph{Esplosione dell’albero di scenario nei modelli multistadio}
Un problema chiaro nei modelli di programmazione stocastica \textbf{multistadio} è la possibile \textbf{esplosione dell’albero di scenario}. Il numero di nodi necessario per rappresentare l’incertezza può diventare un fattore fortemente limitante.\footnote{Nel testo si rimanda a rimedi e approcci alla generazione di scenari nella Sezione 8. Si menziona anche che, nei modelli finanziari, un’ulteriore criticità riguarda la generazione di opportunità di arbitraggio spurie.}

\paragraph{Strutture ad albero e differenza concettuale}
Questo problema è meno rilevante per una struttura ad albero \textbf{lineare} come quella illustrata in Fig.~9. È però importante osservare che una struttura ad albero lineare di questo tipo \textbf{non corrisponde a un modello multistadio}, bensì a un \textbf{modello a due stadi multiperiodale}.

\paragraph{Multiperiod non significa multistage}
\textbf{Multiperiod} non significa \textbf{multistage}. È possibile avere un modello multiperiodale ma \textbf{statico} se tutte le decisioni sono prese \textbf{here-and-now} e poi implementate nei periodi successivi senza adattamento al flusso informativo.  
...
La struttura in Fig.~9 è peculiare: è \textbf{a due stadi}, ma coinvolge \textbf{più periodi}. Apparentemente viola la non-anticipatività, perché dopo la prima diramazione successiva alla radice sarebbe possibile prevedere perfettamente il cammino futuro. Tuttavia, una tale struttura può avere senso quando:
\begin{itemize}[label=-]
...
\subsection{A two-stage multiperiod model: Unit commitment}

\paragraph{Domanda aleatoria come processo discreto nel tempo}
Si consideri un modello stocastico di \textbf{unit commitment}, problema di pianificazione energetica in cui l’incertezza della domanda è modellata come processo stocastico a tempo discreto:
\[
d(\omega)=\big(d_1(\omega),\dots,d_T(\omega)\big),
\qquad
t\in\{1,\dots,T\},\;\omega\in\Omega,
\]
dove $T$ è la lunghezza dell’orizzonte di pianificazione e $\Omega$ è l’insieme (finito) dei cammini campionari. Si denoti con $p_\omega$ la probabilità dello scenario $\omega$.

\paragraph{Classi di generatori e struttura dei costi}
Si dispone di un insieme di $K$ classi di unità generatrici. Una unità della classe $k$ (con $k=1,\dots,K$) può produrre in un intervallo $[L_k,U_k]$ quando è attiva. Per ciascuna classe $k$ sono disponibili $N_k$ unità.

La potenza erogata è una variabile \textbf{semicontinua}: può essere zero, ma se l’unità è attiva deve produrre almeno $L_k$. Se una unità della classe $k$ è attiva al tempo $t$, si sostiene un costo $F_k$ per mantenerla al livello minimo $L_k$, e un costo variabile lineare $c_k$ per ogni unità aggiuntiva di potenza oltre $L_k$. Si sostiene inoltre un costo di avviamento $S_k$ per accendere una unità della classe $k$.\footnote{Per semplicità, non si considerano costi di spegnimento, vincoli di ramp-up, né vincoli di permanenza minima nello stato on/off dopo uno switch.}

\paragraph{Variabili decisionali (primo e secondo stadio)}
Si introducono:
\begin{itemize}[label=-]
    \item $u_{kt}\in\mathbb{Z}_+$: numero totale di unità della classe $k$ attive nel periodo $t$;
    \item $v_{kt}\in\mathbb{Z}_+$: numero di unità della classe $k$ accese all’inizio del periodo $t$;
\end{itemize}
che sono variabili di \textbf{primo stadio}.  
Si introduce inoltre la variabile di \textbf{secondo stadio} (scenario-dipendente):
\[
P^{\omega}_{kt}\ge 0,
\]
che rappresenta la potenza totale erogata dalla classe $k$ nel periodo $t$ nello scenario $\omega$.

\paragraph{Formulazione del modello}
\[
\boxed{
\begin{aligned}
\min\;&
\sum_{k=1}^{K}\sum_{t=1}^{T}\Big(F_k\,u_{kt}+S_k\,v_{kt}\Big)
\;+\;
\sum_{\omega\in\Omega}p_\omega
\left[
\sum_{k=1}^{K}\sum_{t=1}^{T}
c_k\Big(P^{\omega}_{kt}-L_k\,u_{kt}\Big)
\right]
\\[2mm]
\text{s.t. }\;&
\sum_{k=1}^{K}P^{\omega}_{kt}\ge d_t(\omega),
\qquad t=1,\dots,T,\;\omega\in\Omega
\\
& L_k\,u_{kt}\le P^{\omega}_{kt}\le U_k\,u_{kt},
\qquad k=1,\dots,K,\;t=1,\dots,T,\;\omega\in\Omega
\\
& v_{kt}\ge u_{kt}-u_{k,t-1},
\qquad k=1,\dots,K,\;t=1,\dots,T
\\
& u_{kt}\le N_k,
\qquad k=1,\dots,K,\;t=1,\dots,T
\\
& u_{kt}\in\mathbb{Z}_+,\; v_{kt}\in\mathbb{Z}_+,\; P^{\omega}_{kt}\ge 0,
\qquad k=1,\dots,K,\;t=1,\dots,T,\;\omega\in\Omega.
\end{aligned}}
\]

\paragraph{Interpretazione dei vincoli e dell’obiettivo}
La funzione obiettivo (40) comprende:
\begin{itemize}[label=-]
    \item costi di mantenimento al livello minimo e costi di startup, per unità attive (primo stadio);
    \item costi variabili proporzionali alla potenza erogata \emph{oltre} il minimo $L_k$, aggregati in valore atteso rispetto agli scenari (secondo stadio).
\end{itemize}
I vincoli (41) impongono la soddisfazione della domanda in ogni periodo e scenario. I vincoli (42) impongono che la potenza totale della classe $k$ resti tra minimo e massimo, coerentemente con il numero di unità attive. I vincoli (43) definiscono correttamente le variabili di startup (sono nulle quando si spegne o quando il numero di unità resta invariato). I vincoli (44) limitano il numero di unità attive a quelle disponibili.

\paragraph{Osservazioni critiche sul modello}
Si notano due aspetti rilevanti:
\begin{itemize}[label=-]
    \item \textbf{Fattibilità e scenari estremi.} Se il vincolo di domanda (41) è imposto come vincolo rigido in ogni scenario, la soluzione può diventare molto costosa e guidata da pochi scenari improbabili con picchi di domanda elevati. Come nel plant location model, può essere opportuno introdurre una formulazione elastica, ad esempio includendo una unità “di ultima istanza” con capacità illimitata (fornitore esterno) o una penalità per domanda non soddisfatta.
...
\subsection{Scenario generation and stability in stochastic programming}

\paragraph{Ruolo centrale dell’albero di scenario}
L’ingrediente chiave nei modelli di programmazione stocastica multistadio è l’\textbf{albero di scenario} e la qualità della soluzione dipende in modo critico da quanto bene l’incertezza viene catturata. In principio, si potrebbe campionare un modello dinamico dei fattori di rischio tramite simulazione Monte Carlo. Tuttavia, emergono complicazioni aggiuntive.

\paragraph{Complicazioni principali}
\begin{itemize}[label=-]
    \item La dimensione del campione deve rimanere limitata, perché si associano variabili decisionali agli stati e il costo computazionale di risolvere un problema di ottimizzazione stocastica (non solo calcolare una media campionaria) può essere notevole.
    \item Occorre generare decisioni non-anticipative, il che richiede una struttura ad albero soggetta a crescita esponenziale della complessità. Se da ogni nodo si diramano 100 successori, dopo tre passi si hanno un milione di scenari.
    \item Non si sta solo valutando un valore atteso, come nella simulazione Monte Carlo standard, ma si stanno prendendo decisioni risolvendo un problema di ottimizzazione. Qualunque incoerenza o bias nell’albero di scenario verrà sfruttata dal solver, producendo una soluzione apparentemente buona \emph{in-sample} ma probabilmente scarsa \emph{out-of-sample}.
\end{itemize}
È stato dedicato molto sforzo di ricerca alla generazione di scenari; qui se ne possono solo fornire cenni. Qualunque strategia si adotti, è importante verificare la \textbf{stabilità} della soluzione risultante, come discusso brevemente in Sezione~8.1.

\paragraph{Due classi di approcci alla generazione di scenari}
Esistono due classi fondamentali di metodi:
\begin{itemize}[label=-]
    \item \textbf{Generazione stocastica di scenari.} Basata su campionamento casuale Monte Carlo, affidandosi alla “forza bruta” di una grande numerosità campionaria. Questo può essere sostenibile in un problema a due stadi, molto meno in un contesto multistadio. Le prestazioni possono migliorare con tecniche di riduzione della varianza, ad esempio \emph{importance sampling}.
    \item \textbf{Generazione deterministica di scenari.} Invece della forza bruta, si tenta una scelta “intelligente” degli scenari, usando metodi come la quadratura gaussiana, sequenze a bassa discrepanza (es. sequenze di Sobol) o generazione ottimizzata di scenari.
\end{itemize}

\paragraph{Interpretazione come integrazione numerica (quadratura)}
Tutte queste strategie possono essere interpretate nel quadro generale dell’\textbf{integrazione numerica} (quadratura). Si consideri il valore atteso di una funzione $f(x,\tilde{\xi})$, dove $x$ è un vettore di variabili decisionali e $\tilde{\xi}$ un vettore di variabili aleatorie con densità congiunta $\phi(\xi)$. Integrando rispetto a $\xi$ si definisce
\[
F(x) = \mathbb{E}\!\left[f(x,\tilde{\xi})\right].
\]
Questo valore può essere approssimato tramite un campione casuale di dimensione $N$:
\[
F(x) = \int f(x,\xi)\,\phi(\xi)\,d\xi
\;\approx\;
\frac{1}{N}\sum_{B=1}^{N} f(x,\xi^{B}),
\]
dove $\xi^{B}$ è un’osservazione del campione. In alternativa, si può usare un insieme (anche deterministico) di punti $\xi^{B}$ associati a pesi $p_B$:
\[
F(x) \;\approx\; \sum_{B=1}^{N_0} p_B\, f(x,\xi^{B}),
\qquad N_0 < N.
\]
Nel contesto dell’ottimizzazione stocastica, le coppie $(p_B,\xi^{B})$ si interpretano come probabilità e valori di una distribuzione discreta che approssima la distribuzione continua originale. Le formule di quadratura gaussiana forniscono modi per selezionare tale discretizzazione; le sequenze a bassa discrepanza sono usate nella quasi-Monte Carlo per garantire una “buona” copertura del dominio di integrazione.\footnote{Nel testo si cita l’uso di sequenze di Sobol e l’interpretazione come quasi-Monte Carlo.}

\paragraph{Esempio: generazione scenari tramite moment matching (idea)}
Si consideri un vettore aleatorio con distribuzione normale multivariata $\tilde{\xi}\sim \mathcal{N}(\mu,\Sigma)$. L’approssimazione discreta dovrebbe riprodurre le medie e le covarianze della distribuzione continua. Inoltre, per una normale, la skewness e la kurtosis marginali dovrebbero essere rispettivamente 0 e 3.

Indicando con $\xi^{B}_{k}$ la realizzazione della componente $k$ nello scenario $B$ (con $k=1,\dots,m$ e $B=1,\dots,N$), condizioni naturali (con probabilità uniformi) sono:
\[
\frac{1}{N}\sum_{B=1}^{N} \xi^{B}_{k} \approx \mu_k,
\qquad \forall k,
\]
\[
\frac{1}{N}\sum_{B=1}^{N} (\xi^{B}_{k}-\mu_k)(\xi^{B}_{h}-\mu_h) \approx \Sigma_{kh},
\qquad \forall k,h,
\]
\[
\frac{1}{N}\sum_{B=1}^{N} \left(\frac{\xi^{B}_{k}-\mu_k}{\sigma_k}\right)^3 \approx 0,
\qquad \forall k,
\]
\[
\frac{1}{N}\sum_{B=1}^{N} \left(\frac{\xi^{B}_{k}-\mu_k}{\sigma_k}\right)^4 \approx 3,
\qquad \forall k.
\]
Il \emph{moment matching} approssimato si ottiene minimizzando un errore quadratico complessivo rispetto ai valori $\xi^{B}_{k}$, introducendo quattro pesi (uno per ciascun blocco di condizioni) per regolare le prestazioni. Si osserva che il problema di ottimizzazione degli scenari risultante \textbf{non è necessariamente convesso}; tuttavia, una soluzione con errore basso può essere considerata soddisfacente anche se non garantisce ottimalità globale. L’idea può essere generalizzata a \textbf{property matching}, includendo proprietà non necessariamente riconducibili ai momenti.

\paragraph{Critiche al moment matching e alternative basate su distanze}
Il moment (o property) matching è stato criticato perché esistono controesempi di distribuzioni che condividono i primi momenti ma risultano molto diverse. Un’idea alternativa consiste nell’usare metriche che catturino completamente la distanza tra distribuzioni: fissata una topologia dell’albero, si cercano valori e probabilità che minimizzano una distanza rispetto alla distribuzione “vera”. In alternativa, si può partire da un grande albero di scenario e ridurlo a dimensioni gestibili tramite \textbf{scenario reduction} ottimale.
Un contro-argomento alla prospettiva “distanza tra distribuzioni” è che non interessa tanto la distanza tra distribuzione ideale e albero, quanto la \textbf{qualità della soluzione}. Ad esempio, per un problema mean--variance di portafoglio, si può sostenere che basti e avanzi il matching dei primi due momenti.

\paragraph{Linee guida pratiche per la struttura dell’albero}
Si riportano alcune linee guida utili nel progettare la struttura dell’albero:
\begin{itemize}[label=-]
    \item Se l’obiettivo è ottenere una decisione robusta di primo stadio, può essere consigliabile usare un branching ricco al primo stadio e più limitato negli stadi successivi (a patto che, in applicazioni finanziarie, l’albero sia arbitrage-free).
    \item Il numero di stadi può essere limitato usando passi temporali non uniformi (ad esempio, primi due passi di un mese e i successivi di un anno).
    \item Un modo per limitare il numero di stadi consiste nell’augmentare l’obiettivo con un termine che misura la qualità dello stato terminale: ciò può evitare decisioni miopi e consentire alberi più piccoli (si cita un’applicazione all’ATO).
\end{itemize}

\subsection{In- and out-of-sample stability}

\paragraph{Perché serve controllare la stabilità}
Qualunque strategia di generazione scenari venga impiegata, è importante verificare la \textbf{stabilità} della soluzione risultante. Esistono studi sofisticati basati su concetti di spazi metrici; qui si considera un approccio pragmatico.

\paragraph{Problema “esatto” e problema approssimato}
Si denoti in modo compatto il problema stocastico esatto:
\[
\min_{x\in X}\; \mathbb{E}_{P}\!\left[f(x,\tilde{\xi})\right],
\]
dove l’attesa è calcolata rispetto alla misura vera $P$, cioè la distribuzione “esatta” dei fattori di rischio $\tilde{\xi}$.
Il problema che si risolve in pratica usa un albero di scenario approssimato $T$:
\[
\min_{x\in X}\; \widehat{F}(x;T),
\]
dove la notazione $\widehat{F}$ enfatizza che si sta stimando il vero valore atteso dell’obiettivo sostituendo $P$ con l’albero $T$.

\paragraph{Soluzioni indotte da alberi diversi}
Ogni albero $T$ induce una soluzione ottima del problema approssimato. Se si campionano due alberi $T_1$ e $T_2$, si ottengono soluzioni $x^{*}_{1}$ e $x^{*}_{2}$ e corrispondenti valori obiettivo. Se il meccanismo di generazione è affidabile, si dovrebbe osservare stabilità nell’output. È possibile valutare la stabilità delle soluzioni stesse, ma spesso è più semplice e più rilevante verificare la stabilità del \textbf{valore dell’obiettivo}, perché quando l’obiettivo è relativamente piatto possono esistere soluzioni molto diverse con prestazioni comparabili.

\paragraph{In-sample stability}
La stabilità \emph{in-sample} richiede che, campionando due alberi, i valori obiettivo ottenuti non siano troppo diversi:
\[
\widehat{F}(x^{*}_{1};T_{1}) \approx \widehat{F}(x^{*}_{2};T_{2}).
\tag{45}
\]
Questa definizione non si applica direttamente se la generazione dell’albero è deterministica; in tal caso si possono confrontare alberi con strutture di branching leggermente diverse per verificare se la struttura utilizzata sia sufficientemente ricca a garantire stabilità.

\paragraph{Out-of-sample stability}
Poiché l’albero è una rappresentazione limitata dell’incertezza, è essenziale valutare la soluzione in un contesto “reale” in cui può realizzarsi uno scenario differente. La stabilità \emph{out-of-sample} confronta la performance attesa reale delle soluzioni:
\[
\mathbb{E}_{P}\!\left[f(x^{*}_{1},\tilde{\xi})\right]
\approx
\mathbb{E}_{P}\!\left[f(x^{*}_{2},\tilde{\xi})\right].
\tag{46}
\]
Se gli alberi sono affidabili, non si dovrebbe osservare una differenza significativa in performance.

\paragraph{Valutazione out-of-sample: due stadi vs multistadio}
Il controllo out-of-sample è relativamente semplice in un modello a due stadi: si inserisce la soluzione di primo stadio in molti problemi di secondo stadio, ciascuno corrispondente a una realizzazione osservata dei fattori di rischio $\tilde{\xi}$. In genere questo non è troppo oneroso (specie se i secondi stadi sono LP): di regola, risolvere un grande LP stocastico richiede più tempo che risolvere molti LP deterministici.

In un contesto multistadio, invece, la valutazione realistica richiede una costosa simulazione in rolling horizon.

\paragraph{Rolling horizon vs shrinking horizon}
Si distinguono due alternative:
\begin{itemize}[label=-]
    \item \textbf{Sliding/rolling horizon.} Si risolve un problema multistadio con $H$ stadi, si applica la decisione di primo stadio, si osserva una realizzazione out-of-sample dei fattori di rischio nel primo periodo, quindi si risolve di nuovo un problema multistadio con $H$ stadi a partire dal secondo periodo del modello originale.
    \item \textbf{Shrinking horizon.} Si risolve un problema multistadio con $H$ stadi, si applica la decisione di primo stadio, si osserva una realizzazione out-of-sample, e poi si risolve un problema multistadio con $H-1$ stadi; a ogni passo si riduce il numero di stadi.
\end{itemize}
Il secondo approccio ha senso quando l’orizzonte di valutazione è ben definito ed è computazionalmente più economico del primo. In ogni caso, un rolling horizon è tecnicamente fattibile ma costoso.

\paragraph{Test economico alternativo: scambio degli alberi}
Un’alternativa più economica consiste nel generare $x^{*}_{1}$ e $x^{*}_{2}$ da alberi $T_{1}$ e $T_{2}$ e poi valutare i valori obiettivo approssimati scambiando gli alberi, verificando ad esempio:
\[
\widehat{F}(x^{*}_{1};T_{2}) \approx \widehat{F}(x^{*}_{2};T_{1}).
\]

\paragraph{Osservazione finale: limiti operativi dello stochastic programming}
La necessità di valutazione out-of-sample evidenzia un possibile problema dell’approccio di programmazione stocastica: si ottiene una soluzione di primo stadio \emph{here-and-now}, ma non esiste un modo ovvio per adattare le decisioni ai periodi successivi quando l’incertezza si realizza, perché tutte le decisioni sono generate esplicitamente e associate ai nodi dell’albero. Approcci alternativi, come la \textbf{programmazione dinamica} e le \textbf{decision rules}, forniscono invece decisioni in forma di feedback, risultando particolarmente adatti alla simulazione out-of-sample e alla valutazione delle politiche.
...
\[
\mathbb{P}\bigl(\{\omega \mid g(x;\xi(\omega)) \le 0\}\bigr) \ge 1 - \alpha,
\]
dove $g$ è una funzione vettoriale.  
Un punto $\hat{x}$ è ammissibile se l’insieme
\[
S(\hat{x}) = \{\omega \mid g(\hat{x};\xi(\omega)) \le 0\}
\]
ha misura di probabilità almeno pari a $1-\alpha$.

Sia $\mathcal{F}$ il campo di tutti gli eventi e sia $\mathcal{G} \subseteq \mathcal{F}$ la collezione degli eventi con probabilità almeno $1-\alpha$.  
Allora $\hat{x}$ è ammissibile se esiste almeno un evento $G \in \mathcal{G}$ tale che
\[
g(\hat{x};\xi(\omega)) \le 0 \quad \forall \omega \in G,
\]
ossia
\[
\hat{x} \in \bigcap_{\omega \in G} \{x \mid g(x;\xi(\omega)) \le 0\}.
\]

\paragraph{Struttura dell’insieme ammissibile}
L’insieme ammissibile complessivo può quindi essere scritto come
\[
X =
\bigcup_{G \in \mathcal{G}}
\bigcap_{\omega \in G}
\{x \mid g(x;\xi(\omega)) \le 0\}.
\]
Anche se per ogni $\omega$ l’insieme $\{x \mid g(x;\xi(\omega)) \le 0\}$ è convesso, non ci si può aspettare che l’insieme $X$ sia convesso in generale, poiché l’unione di insiemi convessi non è necessariamente convessa.

La convessità del problema complessivo dipende dal tipo di distribuzione (continua o discreta), dalla sua funzione di distribuzione cumulativa, dalla natura dei vincoli (congiunti o disgiunti) e dalla struttura delle funzioni in $g$ (lineari, convesse/concave o arbitrarie).
...
a^T x \le b,
\]
dove $a \sim \mathcal{N}(\mu,\Sigma)$, e si richieda
\[
\mathbb{P}\{a^T x \le b\} \ge \eta.
\]
Per un dato vettore $x$, la variabile casuale $a^T x$ è distribuita come
\[
a^T x \sim \mathcal{N}(\nu,\sigma^2),
\]
dove $\nu = \mu^T x$ e $\sigma^2 = x^T \Sigma x$.

Usando la funzione di distribuzione cumulativa della normale standard $\Phi(\cdot)$, il vincolo può essere riscritto come
\[
\Phi\!\left(\frac{b-\nu}{\sigma}\right) \ge \eta,
\]
ossia
\[
\frac{b-\nu}{\sigma} \ge \Phi^{-1}(\eta).
\]
Assumendo $\eta > 0.5$, quindi $\Phi^{-1}(\eta) > 0$, il vincolo diventa
\[
\mu^T x + \Phi^{-1}(\eta)\,\|\Sigma^{1/2}x\|_2 \le b,
\]
che è un vincolo conico del secondo ordine.  
Ne segue che un problema di programmazione lineare con vincoli probabilistici disgiunti di questo tipo è un problema convesso di tipo SOCP.
...
Q(x;\xi) = \min_{y}
\left\{
q^T y \mid Wy = h(\xi) - T(\xi)x,\; y \ge 0
\right\}.
\]
Il dominio effettivo di $Q(x)$ è costituito dai vettori $x$ ammissibili per i vincoli di primo stadio tali che $Q(x) < +\infty$, ossia per cui il problema di secondo stadio è (quasi sicuramente) ammissibile.
...
\max_{\pi}
\left\{
[h(\xi)-T(\xi)x]^T \pi \mid W^T \pi \le q
\right\}.
\]
Si denoti con
\[
\mathcal{D} = \{\pi \mid W^T \pi \le q\}
\]
l’insieme ammissibile del duale, che è non casuale poiché $q$ è deterministico.
...
\begin{aligned}
Q(x_\lambda)
&= \int Q(x_\lambda;\xi)\, dP \\
&= \int \max_{\pi \in \mathcal{D}}
\left\{
[h(\xi)-T(\xi)x_\lambda]^T \pi
\right\} dP \\
&= \int \max_{\pi \in \mathcal{D}}
\left\{
\lambda [h(\xi)-T(\xi)x_1]^T \pi
+ (1-\lambda)[h(\xi)-T(\xi)x_2]^T \pi
\right\} dP \\
&\le \lambda Q(x_1) + (1-\lambda)Q(x_2).
\end{aligned}
\]
Pertanto la funzione di ricorso è convessa.

\paragraph{Osservazioni finali}
La dimostrazione precedente è semplice poiché l’insieme ammissibile del duale è non casuale. In realtà, la convessità della funzione di ricorso può essere dimostrata in un contesto più generale.

In sintesi:
\begin{itemize}[label=-]
\item la funzione di ricorso è tipicamente continua nel caso di distribuzioni continue;
\item la funzione di ricorso è poliedrica nel caso di distribuzioni discrete;
\item in entrambi i casi è possibile applicare il metodo dei piani di taglio di Kelley;
\item data la struttura del problema, i piani di taglio possono essere generati tramite strategie di decomposizione.
\end{itemize}
