...
\textcolor{Red}{\section{FINIRE CAP 6 -- SLIDE 54/90}}
...
\subsection{Simulazione coda M/M/1 e intervalli di confidenza}
...
Dalla simulazione emergono tre criticità legate all’uso diretto di \texttt{normfit(ww)} sui tempi di attesa, e i tre plot allegati le rendono \textcolor{Indigo}{\textbf{molto evidenti}}.\\
...
\subsection{Simulazione (s,S) con revisione periodica: script \emph{extremely naive}}
...
\hspace*{1.5em}L’obiettivo è capire se la politica sia \textcolor{Indigo}{\textbf{“buona”}} oppure no; tuttavia, in un problema di inventory, il concetto di bontà non è automatico: serve scegliere un criterio di valutazione (ad esempio livello di servizio, vendite perse, costo di mantenimento, costo totale, ecc.).\\
...
\hspace*{1.5em}\textcolor{Indigo}{\textbf{(iii) Ruolo dei grafici diagnostici.}} Tracciando le variabili nel tempo (ad esempio \texttt{onHand}, \texttt{ordered} o \texttt{lostSales}) si ottiene una comprensione qualitativa della dinamica del sistema: oscillazioni della scorta, frequenza dei riordini e presenza (o assenza) di stockout. Tuttavia, tali grafici servono solo come \textbf{strumenti esplorativi} e non sostituiscono una vera analisi delle prestazioni.\\
...
\subsection{Politica (s,S) a revisione periodica: simulazione su scenari e ottimizzazione dei parametri (3a--3c)}
...
\begin{figure}[H]
\centering
\includegraphics[height=2.8cm]{Figures/4.pdf}
...
\subsection{Coda M/M/1 con balking (FIFO): simulazione percentuale di clienti che rinunciano (4a--4b)}
...
while count <= toServe
    ...
    if nextCompletion <= nextArrival
        ...
        if numInQueue == 0
            % nessuno in coda: il server diventa idle
            nextCompletion = inf;
        else
...
\hspace*{1.5em}\textcolor{Indigo}{\textbf{(ii) Struttura a eventi e stato.}} La versione funzione (4b) è più pulita perché incapsula lo stato e separa la logica dei due eventi, rendendo più facile estendere il modello (ad esempio aggiungendo nuove regole o nuove metriche).\\
...
\subsection{Two-stage ATP/ATO: modello ai valori attesi vs modello stocastico (5a--5b)}
...
\subsection{Capacity control e tariffe aeree: protezione della classe alta e simulazione degli arrivi (6)}

\paragraph{Contesto del problema}
Lo script modella un problema di \textbf{revenue management} per un volo con capacità limitata: esistono due classi tariffarie con prezzi diversi (classe 1 più costosa, classe 2 più economica) e le richieste arrivano nel tempo secondo processi di Poisson.\\
...
\hspace*{1.5em}Il livello di protezione viene calcolato tramite un quantile Poisson: si usa \texttt{poissinv} sul numero atteso di arrivi della classe 1 nell’intero orizzonte $T$.
...
\subsection{Roll queue: simulazione a eventi discreti con buffer e domanda batch (7)}
...
\subsection{Cutting stock: formulazione simmetrica e generazione di colonne (8a--8c)}
...
\hspace*{1.5em}In questa sezione vengono confrontati tre approcci:
