...
\(
\mathbf{Q}(x) = \mathbb{E}_{\mathbb{P}}\!\left[Q(x,\tilde{\xi})\right].
\)
...
& \sum_{i\in\mathcal{P}} f_i y_i 
+ \sum_{s\in S} \pi^s 
\sum_{i\in\mathcal{P}} \sum_{j\in\mathcal{D}} c_{ij} x_{ij}^s \\[0.4em]
\text{s.t.} \quad
& \sum_{i\in\mathcal{P}} x_{ij}^s = d_j^s
\qquad s\in S,\, j\in\mathcal{D} \\[0.3em]
& \sum_{j\in\mathcal{D}} x_{ij}^s \le R_i y_i
\qquad s\in S,\, i\in\mathcal{P} \\[0.3em]
& x_{ij}^s \ge 0,\; y_i \in \{0,1\}.
\end{align*}
...
\paragraph{Recap}
I \textcolor{BurntOrange}{\textbf{fondamenti microeconomici del pricing}} chiariscono perché il \textbf{prezzo} rappresenti una leva decisionale centrale ma non autonoma, poiché interagisce in modo sistematico con \textbf{domanda}, \textbf{costi}, \textbf{concorrenza} e \textbf{vincoli operativi}.
Un modello elementare di profitto consente di isolare i principali driver economici e di evidenziare come variazioni relativamente piccole del prezzo possano produrre effetti amplificati sul profitto, soprattutto rispetto a leve più lente o costose come la riduzione dei costi o gli investimenti in marketing.
\\ \hspace*{1.5em}
Questa rappresentazione elementare è però insufficiente, poiché \textbf{prezzo e volume sono endogeni} e legati da relazioni causali complesse.
Il prezzo influenza direttamente la domanda, ma anche indirettamente i costi, le reazioni dei concorrenti, la quota di mercato e le decisioni dei partner lungo la supply chain.
La comprensione di tali \textcolor{BurntOrange}{\textbf{catene causali}} è essenziale per evitare interpretazioni fuorvianti e per valutare correttamente l’impatto di una politica di pricing sul profitto complessivo.
\\ \hspace*{1.5em}
La massimizzazione del profitto emerge come obiettivo naturale, ma non coincide né con la sola massimizzazione dei ricavi né con la minimizzazione dei costi.
In contesti caratterizzati da costi marginali bassi o trascurabili, come nei servizi digitali o nel trasporto aereo, diventa razionale focalizzarsi su strategie di \textcolor{BurntOrange}{\textbf{revenue e yield management}}, che sfruttano la variabilità della domanda nel tempo e l’eterogeneità dei consumatori.
Il prezzo non è quindi un semplice parametro, ma uno strumento di allocazione della capacità e di gestione del rischio.
\\ \hspace*{1.5em}
La relazione tra prezzo e quantità è formalizzata tramite \textcolor{BurntOrange}{\textbf{funzioni di domanda}}.
Il modello lineare fornisce un primo riferimento intuitivo e consente di derivare risultati chiari sulla massimizzazione di ricavi e profitti.
Il confronto tra \textcolor{BurntOrange}{ricavo marginale} e \textcolor{BurntOrange}{costo marginale} chiarisce il meccanismo che determina la quantità ottima e mette in luce il ruolo della forma della funzione di costo.
Economie e diseconomie di scala influenzano direttamente la concavità del profitto e quindi la stabilità della soluzione ottima.
\\ \hspace*{1.5em}
La sola pendenza della domanda non è tuttavia sufficiente a descrivere il comportamento dei consumatori.
Una misura economicamente significativa è l’\textcolor{BurntOrange}{\textbf{elasticità di prezzo della domanda}}, che quantifica la sensibilità relativa della quantità rispetto al prezzo.
L’elasticità distingue regimi di domanda elastica e anelastica e fornisce criteri operativi per valutare se una variazione di prezzo aumenti o riduca i ricavi.
Nel caso di domanda a elasticità costante, il legame tra elasticità e ricavo diventa particolarmente trasparente e consente una lettura immediata delle politiche di prezzo.
\\ \hspace*{1.5em}
L’interpretazione economica dei modelli di domanda è ricondotta al concetto di \textcolor{BurntOrange}{\textbf{willingness-to-pay}}, che rappresenta il prezzo di riserva individuale dei consumatori.
La domanda aggregata può essere vista come l’integrale della distribuzione della willingness-to-pay sulla popolazione.
Da questa prospettiva, il \textbf{consumer surplus} misura l’utilità non estratta dall’impresa ed evidenzia le perdite associate all’applicazione di un prezzo unico.
Il consumer surplus diventa quindi una grandezza chiave per comprendere i limiti strutturali del pricing uniforme.
\\ \hspace*{1.5em}
La possibilità di estrarre almeno parzialmente il consumer surplus motiva l’adozione di \textcolor{BurntOrange}{\textbf{strategie di discriminazione di prezzo}}.
Attraverso segmentazione diretta, segmentazione indiretta, versioning, bundling, abbonamenti e razionamento, l’impresa può migliorare il profitto e, in alcuni casi, includere consumatori che sarebbero altrimenti esclusi dallo scambio.
La discriminazione di prezzo non garantisce esiti Pareto-migliorativi, ma costituisce uno strumento fondamentale del pricing moderno.
\\ \hspace*{1.5em}
Per modellare in modo più realistico il comportamento di scelta, la domanda viene collegata a modelli probabilistici.
La \textcolor{BurntOrange}{\textbf{funzione logit}} fornisce una forma flessibile e interpretabile, coerente con una distribuzione plausibile della willingness-to-pay.
Questo approccio conduce ai \textcolor{BurntOrange}{\textbf{modelli di scelta discreta}}, nei quali i consumatori selezionano un’alternativa massimizzando un’utilità composta da una parte deterministica e una parte casuale.
\\ \hspace*{1.5em}
Il \textcolor{BurntOrange}{\textbf{multinomial logit (MNL)}} emerge come modello di riferimento per la scelta tra alternative multiple.
Esso è semplice da stimare, ampiamente utilizzato in marketing e machine learning, e facilmente integrabile in problemi di ottimizzazione.
Il modello presenta tuttavia un limite strutturale rilevante, l’\textbf{indipendenza dalle alternative irrilevanti (IIA)}, che impone cautela nell’interpretazione dei risultati e motiva l’uso di estensioni più sofisticate in presenza di forti correlazioni tra alternative.
\\ \hspace*{1.5em}
Le decisioni di pricing sono infine collocate in un contesto \textbf{strategico}, in cui più decisori interagiscono in modo non cooperativo.
I modelli di \textcolor{BurntOrange}{Cournot} e \textcolor{BurntOrange}{Bertrand} mostrano come la competizione su quantità o prezzi conduca a esiti profondamente diversi, anche in presenza di strutture di costo simili.
La differenziazione dei prodotti, modellata tramite il \textcolor{BurntOrange}{modello di Hotelling}, attenua l’undercutting e consente l’esistenza di equilibri con prezzi positivi.
\\ \hspace*{1.5em}
La distinzione tra giochi simultanei e sequenziali mette in evidenza il ruolo dell’informazione e del timing decisionale.
Nel modello di \textcolor{BurntOrange}{von Stackelberg}, il leader sfrutta la conoscenza della reazione del follower per ottenere un vantaggio strategico, pur riducendo il profitto complessivo rispetto alla soluzione coordinata.
In contesti verticali, la decentralizzazione delle decisioni di prezzo genera il fenomeno della \textcolor{BurntOrange}{\textbf{doppia marginalizzazione}}, che porta a prezzi più elevati, domanda ridotta e perdita di efficienza lungo la filiera.
...
Nel seguito si considera l’\textcolor{BurntOrange}{\textbf{Nash equilibrium}} nel caso in cui a ciascun giocatore sia disponibile un \textbf{continuo di azioni} (un insieme infinito di azioni). Si analizza innanzitutto il comportamento di \textbf{due imprese} che competono in termini di \textbf{quantità}: entrambe vorrebbero massimizzare il proprio \textbf{profitto}, ma si influenzano reciprocamente poiché le quantità prodotte incidono sul \textbf{prezzo} a cui il prodotto viene venduto sul mercato. Tale prezzo è \textbf{comune} a entrambe le imprese, poiché si assume che producano un bene \textbf{perfettamente identico}. Questo tipo di competizione è detto \textcolor{BurntOrange}{\textbf{competizione di Cournot}}; il caso in cui le imprese competono sui \textbf{prezzi} è detto \textcolor{BurntOrange}{\textbf{competizione di Bertrand}}. Nel caso di competizione sui prezzi, si deve inoltre distinguere tra prodotti \textbf{omogenei} e prodotti \textbf{differenziati}.

\paragraph{Competizione di Cournot}
Un gioco con \textbf{mosse simultanee}, in cui le azioni sono \textbf{quantità}, conduce all’\textcolor{BurntOrange}{\textbf{equilibrio di Cournot--Nash}}. Questo tipo di modello è rilevante in mercati come quello dell’\textbf{energia}, nei quali le imprese selezionano le quantità prodotte mentre i prezzi vengono determinati da meccanismi d’asta potenzialmente complessi.
\\ \hspace*{1.5em}
Per chiarire il concetto, si considera un modello estremamente semplice, nel quale si assume che ciascuna impresa abbia una struttura di costo composta esclusivamente da un \textbf{costo variabile}
\[
TC_i(q_i)=c_i q_i, \qquad i=1,2.
\]
Qui $TC_i$ denota il \textbf{costo totale} dell’impresa $i$, $c_i$ è il \textbf{costo variabile unitario} e $q_i$ è la quantità prodotta dall’impresa $i=1,2$.
\\ \hspace*{1.5em}
La quantità totale offerta sul mercato è
\(
Q=q_1+q_2,
\)
e influenza il prezzo secondo una \textbf{funzione di domanda inversa lineare}
\[
P(Q)=a-bQ, \qquad a,b>0,\; a\ge c_i.
\]
Questo modello stilizzato assume implicitamente che \textbf{tutti i beni prodotti vengano venduti sul mercato}. Il profitto dell’impresa $i$ è quindi
\[
\pi_i(q_1,q_2)=P(q_1+q_2)q_i-TC_i(q_i)
=[a-b(q_1+q_2)]q_i-c_i q_i, \qquad i=1,2.
\]
\hspace*{1.5em}
Assumendo che le due imprese prendano le loro decisioni \textbf{simultaneamente}, con \textbf{informazione completa} e \emph{common knowledge}, si pone il problema di determinare l’\textbf{equilibrio di Nash}. L’equilibrio si ottiene individuando le \textbf{funzioni di risposta ottima} $R_i(q_j)$. La condizione di stazionarietà del profitto dell’impresa~1 fornisce
\[
\frac{\partial \pi_1(q_1,q_2)}{\partial q_1}
=a-2bq_1-bq_2-c_1=0
\;\Rightarrow\;
R_1(q_2)=\frac{a-c_1}{2b}-\frac{1}{2}q_2.
\]
Analogamente, per l’impresa~2 si ottiene
\[
R_2(q_1)=\frac{a-c_2}{2b}-\frac{1}{2}q_1.
\]
\hspace*{1.5em}
Per risolvere il problema si deve individuare il punto di intersezione delle due funzioni di risposta; in altre parole, si risolve il sistema
\[
\begin{cases}
q_1^c=R_1(q_2^c),\\
q_2^c=R_2(q_1^c),
\end{cases}
\]
dove il pedice $c$ indica l’\textbf{equilibrio di Cournot}. Le funzioni di risposta risultano essere \textbf{rette a pendenza negativa}.
\\ \hspace*{1.5em}
Risolvendo il sistema lineare
\[
\begin{cases}
q_1^c=\dfrac{a-c_1}{2b}-\dfrac{1}{2}q_2^c,\\[0.6em]
...
\qquad
q_2^c=\frac{a-2c_2+c_1}{3b}.
\]
Il \textbf{prezzo di equilibrio} risulta
\[
p^c=\frac{a+c_1+c_2}{3},
\]
e il profitto di ciascuna impresa è
\[
\pi_i^c=(p^c-c_i)q_i^c
=\left(\frac{a+c_i+c_j}{3}-c_i\right)\left(\frac{a-2c_i+c_j}{3b}\right)
=\frac{(a-2c_i+c_j)^2}{9b}
=b\,(q_i^c)^2.
\]
\\ \hspace*{1.5em}
Se un’impresa riesce a \textbf{ridurre il proprio costo}, essa aumenta sia la quantità prodotta sia il profitto. Nel caso in cui le due imprese abbiano la \textbf{stessa tecnologia produttiva}, ossia $c_1=c_2$, si ottiene una \textbf{soluzione simmetrica} $q_1^c=q_2^c$, come atteso.

\paragraph{Competizione di Bertrand}
Si considerino due imprese che competono sui \textbf{prezzi} per un \textbf{bene omogeneo}, sotto l’ipotesi semplificativa di \textbf{domanda} e \textbf{funzioni di costo lineari}.
Per analizzare il problema è necessario specificare in modo chiaro come i consumatori reagiscono ai prezzi.
Poiché il prodotto è omogeneo, è ragionevole assumere che i consumatori acquistino dal venditore con il \textbf{prezzo più basso}; se i prezzi coincidono, si assume che il mercato venga \textbf{diviso equamente} tra le due imprese.
In questa analisi non si considerano \textbf{vincoli di capacità}.
\\ \hspace*{1.5em}
La quantità venduta dall’impresa $i$ (con $i=1,2$ e $j=2,1$ che denota il concorrente) è data da
\[
q_i =
\begin{cases}
...
\end{cases}
\]
\hspace*{1.5em}L’idea è che non vi sia \textbf{domanda} quando il prezzo di un’impresa supera il \textbf{prezzo limite} $\alpha/\beta$ o quando è strettamente maggiore del prezzo del concorrente.
La domanda viene divisa equamente quando i prezzi coincidono (e sono inferiori al limite), mentre in tutti gli altri casi l’impresa più economica \textbf{cattura l’intera domanda}.
\\ \hspace*{1.5em}
Un \textcolor{BurntOrange}{\textbf{equilibrio di Bertrand--Nash}} è una quadrupla di prezzi e quantità
\(
(p_1^b,p_2^b,q_1^b,q_2^b)
\)
tale che:
\begin{enumerate}[label=\arabic*.]
\item $p_1^b$ risolve il problema
\(
\max_{p_1}\ \pi_1(p_1,p_2^b)=(p_1-c_1)q_1,
\)
per $p_2=p_2^b$.
\item $p_2^b$ risolve il problema
\(
\max_{p_2}\ \pi_2(p_1^b,p_2)=(p_2-c_2)q_2,
\)
per $p_1=p_1^b$.
\item Le quantità $q_1$ e $q_2$ sono date dalla equazione precedente.
\end{enumerate}
È chiaro che i prezzi non vengono fissati al di sotto dei \textbf{costi marginali} $c_1$ e $c_2$, rispettivamente.
\\ \hspace*{1.5em}
Il concetto chiave alla base dell’equilibrio di Bertrand--Nash è l’\textcolor{BurntOrange}{\textbf{undercutting}}.
Se $c_1<c_2$, l’impresa~1 può sottoquotare l’impresa~2 fissando un prezzo strettamente inferiore a $p_2$.
In senso stretto, \emph{non esiste un equilibrio}, poiché non esiste un massimo ma soltanto un \emph{supremo}.
A differenza della competizione di Cournot, si manifesta una sorta di \textbf{discontinuità}: una riduzione infinitesima del prezzo può indurre un \textbf{salto discreto della domanda} per un’impresa.
\\ \hspace*{1.5em}
Tuttavia, si può mostrare che esiste un equilibrio quando il \textbf{costo marginale} è lo stesso per entrambe le imprese, pari a $c$:
\[
p_1=p_2=c,
\qquad
q_1=q_2=\frac{\alpha-\beta c}{2}.
\]
Il risultato si dimostra facilmente per \textbf{contraddizione}: per qualunque altra configurazione dei prezzi, almeno una delle imprese avrebbe incentivo a deviare.
\\ \hspace*{1.5em}
Il risultato netto è che può \textbf{non esservi alcun profitto} per nessuna delle due imprese.
Se i costi marginali sono differenti, l’esistenza di un massimo (e quindi di un equilibrio) richiede che i prezzi possano variare solo di una quantità discreta $\varepsilon$ (ad esempio un centesimo di euro).
Se $c_2-c_1>\varepsilon$, l’impresa~1 può sottoquotare fissando
\(
p_1=p_2-\varepsilon,
\)
e l’equilibrio risultante è
\(
p_2=c_2,\qquad
p_1=c_2-\varepsilon,\qquad
q_1=\alpha-\beta(c_2-\varepsilon),\qquad
q_2=0.
\)
\\ \hspace*{1.5em}
Tralasciando gli aspetti tecnici, questa previsione risulta in parte in contrasto con le \textbf{evidenze empiriche}.
Nella pratica, la struttura dei costi non è puramente lineare, esistono \textbf{vincoli di capacità} e i beni non sono realmente omogenei; possono inoltre emergere ulteriori complicazioni, tali per cui non è necessario che sul mercato operi una sola impresa.
Osservando il problema da un’altra prospettiva, questo risultato suggerisce che le imprese dovrebbero \textbf{differenziare le proprie offerte} e sfruttare la \textbf{disomogeneità dei consumatori}.

\paragraph{Competizione di prezzo per beni non omogenei: modello di Hotelling}
Per trattare beni \textbf{non omogenei} (differenziati) è necessario introdurre un \textbf{modello di scelta del consumatore}.
Si considera un modello \textbf{unidimensionale} delle preferenze dei consumatori.
I beni prodotti da due imprese, $A$ e $B$, sono caratterizzati da una singola \textbf{caratteristica}, misurata su una scala continua da $0$ a $L$.
Per semplicità, si trascurano i \textbf{costi di produzione}.
\\ \hspace*{1.5em}
L’impresa $A$ produce un bene con caratteristica al livello $a$, mentre l’impresa $B$ produce un bene con caratteristica al livello $L-b$, a distanza $b$ dal valore massimo.
Questa rappresentazione geometrica è nota come \textcolor{BurntOrange}{\textbf{modello della strada lineare di Hotelling}}.
\\ \hspace*{1.5em}
Ogni consumatore ha un livello preferito della caratteristica, rappresentato dalla sua \textbf{posizione} lungo la strada.
Si assume che i consumatori siano \textbf{uniformemente distribuiti} sull’intervallo $[0,L]$.
Ciascun consumatore ha un marchio preferito, ossia quello più vicino ai propri gusti.
Se i due prezzi $p_A$ e $p_B$ coincidono, il consumatore acquista semplicemente il marchio più vicino.
Tuttavia, esiste un \textbf{trade-off tra prezzo e soddisfazione}.
\\ \hspace*{1.5em}
Si assume che la (dis)utilità di ciascun consumatore possa essere espressa tramite un \textbf{costo di trasporto} $\tau$.
Pertanto, per un consumatore situato in posizione $x$ sulla scala, l’utilità è
\[
u(x)=
\begin{cases}
-p_A-\tau\,|x-a| & \text{se sceglie il marchio } A,\\[0.3em]
-p_B-\tau\,|x-(L-b)| & \text{se sceglie il marchio } B.
\end{cases}
\]
\\ \hspace*{1.5em}
Si individua quindi un \textbf{consumatore critico}, localizzato in $x_i$ con $a<x_i<L-b$, che è \textbf{indifferente} tra i due marchi:
\[
-p_A-\tau(x_i-a)=-p_B-\tau(L-b-x_i),
\]
da cui si ottiene
\[
x_i=\frac{p_B-p_A}{2\tau}+\frac{L-b+a}{2}.
\]
Come verifica di coerenza, si osserva che quando i prezzi sono uguali, $x_i$ coincide con il \textbf{punto medio} tra le due localizzazioni dei marchi.
L’impresa $A$ cattura tutta la domanda nell’intervallo $[0,x_i)$, per cui la sua funzione di domanda è pari a $x_i$, mentre la domanda per il marchio $B$ è
\[
L-x_i=\frac{p_A-p_B}{2\tau}+\frac{L+b-a}{2}.
\]
\\ \hspace*{1.5em}
Ci si può chiedere se esista un \textbf{equilibrio di Bertrand--Nash nei prezzi}, a localizzazioni fissate.
L’impresa $A$, dato $p_B$, risolve il problema
\[
\max_{p_A}\ \pi_A
=
\frac{p_Bp_A-p_A^2}{2\tau}
+\frac{L-b+a}{2}\,p_A,
\]
...
\max_{p_B}\ \pi_B
=
\frac{p_Ap_B-p_B^2}{2\tau}
+\frac{L+b-a}{2}\,p_B.
\]
Le \textbf{condizioni di stazionarietà del primo ordine} sono
\[
...
=0,
\]
da cui si ottengono i \textbf{prezzi di equilibrio}
\[
p_A^e=\frac{\tau(3L-b+a)}{3},
\qquad
p_B^e=\frac{\tau(3L+b-a)}{3}.
\]
Il \textbf{profitto} dell’impresa $A$ è
\[
\pi_A^e=x_i^e\,p_A^e
=\frac{\tau(3L-b+a)^2}{18}.
\]
Nel \textbf{caso simmetrico} $a=b$, che non implica che le localizzazioni coincidano, i prezzi sono uguali e il profitto si riduce a
\[
\frac{\tau L^2}{2}
\]
per entrambe le imprese.
Si osserva che \textbf{quanto maggiore è il costo di trasporto, tanto maggiori sono i prezzi}.
Questo risultato si estende anche a contesti con \textbf{switching costs}.
\\ \hspace*{1.5em}
Non si deve tuttavia dare per scontata l’esistenza di una coppia di prezzi di equilibrio strettamente positivi.
Si possono mostrare i seguenti risultati
\begin{enumerate}[label=\arabic*.]
\item Se i marchi sono \textbf{omogenei} (nello stesso punto), allora $p_A^e=p_B^e=0$, a causa dell’\textbf{undercutting}.
\item Se i marchi non sono \textbf{troppo vicini}, valgono gli equilibri precedenti; altrimenti, non esiste equilibrio.
\item Non esiste equilibrio nel gioco in cui sia le \textbf{localizzazioni} sia i \textbf{prezzi} sono variabili decisionali.
\end{enumerate}

\paragraph{Esistenza dell’equilibrio nel modello di Hotelling}
Si consideri il \textbf{caso simmetrico}, in cui $a=b$.
Sulla base della discussione precedente, ci si aspetterebbe
\[
p_A^e=p_B^e=\tau L,
\qquad
\pi_A^e=\pi_B^e=\frac{\tau L^2}{2}.
\]
In realtà, si può dimostrare che questo risultato vale \textbf{solo se} $a\le L/4$.
\\ \hspace*{1.5em}
Si assuma che $p_B=\tau L$, ossia il prezzo di equilibrio, e si modifichi $p_A$.
L’impresa $A$ cattura l’intero mercato se persino il consumatore localizzato in $L-b$ sceglie $A$.
Ciò accade se il prezzo $p_A$ più il costo di trasporto è inferiore a $p_B=\tau L$.
Osservando che la distanza tra $A$ e $B$ è $L-2a$, l’impresa $A$ cattura il mercato se
\(
p_A<\tau L-\tau(L-2a)=2a\tau=:p_{A,\mathrm{lim}}.
\)
In questo caso, il profitto di $A$ è una \textbf{funzione lineare crescente} di $p_A$, e il suo valore limite è
\(
\pi_A^\circ=p_{A,\mathrm{lim}}\,L=2a\tau L.
\)
\\ \hspace*{1.5em}
Al prezzo limite emerge un’ambiguità, poiché i consumatori alla destra di $B$ risultano indifferenti.
Si può assumere che tale segmento di mercato venga suddiviso in parti uguali.
In ogni caso, il profitto di $A$ presenta una \textbf{discontinuità} come funzione del prezzo.
\\ \hspace*{1.5em}
Per prezzi superiori a $p_{A,\mathrm{lim}}$, il mercato si divide e, dalla Eq.~(13), si ottiene
\[
\pi_A=
...
\]
In equilibrio, quando $p_A=\tau L$, ciò si riduce a
\(
\pi_A^e=\frac{\tau L^2}{2}.
\)
Quando invece $p_A$ diventa sufficientemente grande, l’impresa $B$ cattura l’intero mercato e il profitto di $A$ si annulla.
\\ \hspace*{1.5em}
Esiste un equilibrio (ossia esiste un \textbf{massimo} e non solo un \emph{supremo}) se
\(
\pi_A^e\ge \pi_A^\circ,
\)
...
a\le \frac{L}{4}.
\]
Questo implica che le imprese devono essere \textbf{sufficientemente distanti} l’una dall’altra affinché l’equilibrio esista.

\paragraph{Giochi simultanei e sequenziali}
Finora si è assunto che le due imprese in concorrenza giochino \textbf{simultaneamente} e debbano prendere decisioni analoghe.
In alcuni contesti, tuttavia, è più naturale assumere che uno dei due giocatori si muova \textbf{per primo}.
Ad esempio, un produttore può dover decidere il \textbf{prezzo all’ingrosso}, mentre un rivenditore sceglie la \textbf{quantità acquistata}, assumendo che il prezzo di vendita al dettaglio sia fissato.
\\ \hspace*{1.5em}
Si può quindi chiedere che cosa accada nel gioco sulle quantità se si assume che l’impresa~1, il \textbf{leader}, fissi la propria quantità $q_1$ prima dell’impresa~2, il \textbf{follower}.
A differenza del gioco simultaneo, l’impresa~2 conosce la decisione dell’impresa~1 prima di scegliere, e dispone dunque di \textbf{informazione perfetta}.
L’analisi del gioco sequenziale risultante conduce all’\textcolor{BurntOrange}{\textbf{equilibrio di von Stackelberg}}.
\\ \hspace*{1.5em}
L’impresa~1 prende la propria decisione conoscendo la \textbf{funzione di risposta ottima} dell’impresa~2, data dall’Eq.~(8).
Il problema del leader è quindi
\[
\max_{q_1}\ \pi_1^s
=
P\!\bigl(q_1+R_2(q_1)\bigr)q_1-c_1q_1
=
\Bigl[a-b\Bigl(q_1+\frac{a-c_2}{2b}-\frac{q_1}{2}\Bigr)\Bigr]q_1-c_1q_1,
\]
dove il pedice superiore “$s$” indica la competizione di von Stackelberg.
Applicando la condizione di stazionarietà si ottiene
\[
q_1^s=\frac{a-2c_1+c_2}{2b}
=\frac{3}{2}\,q_1^c.
\]
Ne segue che l’impresa~1 \textbf{produce di più} nel gioco sequenziale rispetto al gioco di Cournot.
\\ \hspace*{1.5em}
Sostituendo questo valore nella funzione di risposta ottima $R_2(q_1)$, si ottiene
\[
q_2^s
=
\frac{a-c_2}{2b}
...
\frac{a-3c_2+2c_1}{4b}
=
\frac{a-2c_2+c_1+(c_1-c_2)}{4b}
=
\frac{3}{4}\,q_2^c+\frac{c_1-c_2}{4b}.
\]
La produzione dell’impresa~2 è quindi una \textbf{frazione} di quella ottenuta nel gioco di Cournot, più un termine che è \textbf{positivo} se l’impresa~1 è meno efficiente dell’impresa~2.
\\ \hspace*{1.5em}
Risulta infine interessante confrontare i \textbf{profitti} delle due imprese in questo tipo di gioco.
Il confronto è particolarmente semplice quando i costi marginali di produzione coincidono; l’idea può essere illustrata tramite un semplice esempio numerico.

\paragraph{Esempio}
Dall’esempio emerge che la \textbf{collusione} massimizza il profitto complessivo, poiché l’output viene ridotto e il prezzo aumenta rispetto ai casi competitivi.
Nel gioco di \textbf{Cournot}, la concorrenza simultanea porta a una produzione maggiore e a profitti totali inferiori rispetto al monopolio.
Nel gioco \textbf{sequenziale di von Stackelberg}, il prezzo scende ulteriormente e il profitto complessivo diminuisce, ma il \textbf{leader ottiene un vantaggio strategico}, realizzando un profitto più elevato rispetto al caso simultaneo, a scapito del follower.

\paragraph{È sempre conveniente muoversi per primi?}
Dall’esempio si evince che il vantaggio di muoversi per primi \textbf{non è garantito in generale}.
Nel gioco sequenziale standard il leader non può fare peggio del caso simultaneo, poiché può sempre replicarne la scelta.
Tuttavia, in presenza di \textbf{asimmetrie informative} o di \textbf{incertezza}, la mossa iniziale può rivelare informazioni utili al follower, riducendo o annullando il vantaggio del leader.
In alcuni contesti strategici, come mostrato nel secondo esempio, \textbf{nessun giocatore desidera muoversi per primo}, poiché ciò può esporlo a esiti sfavorevoli.

\paragraph{Pricing e doppia marginalizzazione}
Si consideri un contesto B2B in cui un produttore e un rivenditore interagiscono tramite i prezzi.
Si confrontano le decisioni di pricing e il profitto complessivo della filiera in due assetti: integrazione verticale e controllo decentralizzato.
In entrambi i casi si assume una domanda lineare
\(
d(p)=1-p,
\)
dove $p$ è il prezzo di mercato, e una struttura dei costi lineare con costo marginale $c<1$.
\\ \hspace*{1.5em}
Nel caso di integrazione verticale, un unico decisore controlla produzione e distribuzione e risolve il problema
\(
\max_{p}\ (p-c)(1-p).
\)
La soluzione fornisce il prezzo ottimo, la domanda e il profitto
\[
p_{vi}^*=\frac{1+c}{2},
\qquad
d_{vi}^*=\frac{1-c}{2},
...
\]
\hspace*{1.5em}
Nel caso di filiera non integrata, ciascun attore fissa un prezzo.
Si assume un gioco sequenziale in cui il produttore è leader e fissa il prezzo all’ingrosso $p_w$, mentre il rivenditore sceglie il prezzo di mercato $p_m$ risolvendo
\(
...
\]
Il problema del produttore diventa quindi
\[
...
p_{w,dec}^*=\frac{1+c}{2}.
\]
Il prezzo finale sul mercato risulta
\[
p_{m,dec}^*=\frac{3+c}{4},
\]
che è maggiore rispetto al caso integrato, mentre la domanda è inferiore ed è data da
\[
d_{dec}^*=\frac{1-c}{4}.
\]
\hspace*{1.5em}
Il profitto complessivo della filiera decentralizzata è la somma dei profitti del produttore e del rivenditore,
\[
\pi_{dec}^*
=
(p_{w,dec}^*-c)d_{dec}^*
+
(p_{m,dec}^*-p_{w,dec}^*)d_{dec}^*
=
\frac{3(1-c)^2}{16},
\]
che risulta strettamente inferiore a $\pi_{vi}^*$.
Questo fenomeno prende il nome di \textbf{\textcolor{BurntOrange}{doppia marginalizzazione}} ed è dovuto al fatto che ciascun attore applica un \textbf{markup} sul costo marginale percepito, che cresce lungo la filiera.
In presenza di incertezza sulla domanda, tali inefficienze possono essere attenuate mediante politiche di pricing contrattuali, come tariffe a due parti o contratti di buyback, finalizzate al coordinamento della supply chain.

\subsection{Discrete choice models}

\paragraph{Conjoint analysis}
La \textbf{conjoint analysis} è un metodo ampiamente utilizzato nel marketing (e in altri ambiti) per la progettazione di prodotti e servizi, per la valutazione dell’impatto del prezzo e per la costruzione di simulatori di mercato.
L’idea di base è che i consumatori reagiscano ad alternative descritte come combinazioni di valori di attributi.
\\ \hspace*{1.5em}
Esistono diverse varianti della conjoint analysis, ma le principali sono la \emph{ratings-based conjoint analysis}, di tipo tradizionale, e la \emph{choice-based conjoint analysis}, che rappresenta l’approccio più diffuso.
\\ \hspace*{1.5em}
Nel caso della ratings-based conjoint analysis, il punteggio di preferenza $Y_i$ associato all’alternativa $i$ può essere modellato come
\[
Y_i = \sum_{k=1}^{m} U_k(x_{ik}) + \varepsilon_i,
\]
dove ciascuna funzione $U_k$ rappresenta il \textbf{partworth} dell’attributo $k$, mentre $\varepsilon_i$ è un termine di errore.
Si osserva che chiedere ai consumatori di stabilire una priorità tra gli attributi è in genere poco informativo, poiché tutti gli attributi risultano rilevanti; la richiesta di un punteggio $Y_i$ costituisce invece un’opzione più efficace e si presta naturalmente all’uso di modelli di regressione classici, eventualmente basati su variabili dummy.
\\ \hspace*{1.5em}
La conjoint analysis classica consente di inferire i partworth a partire dai punteggi assegnati dai consumatori a un insieme di alternative, quali stimoli o profili.
Il carattere additivo del modello costituisce una chiara limitazione; tuttavia, è possibile introdurre interazioni tra attributi, che possono risultare particolarmente rilevanti per specifiche combinazioni di valori, come nel caso di attributi fortemente complementari.
\\ \hspace*{1.5em}
Tuttavia, l’assegnazione di punteggi non riflette il modo in cui i consumatori effettuano effettivamente le proprie scelte.
Per costruire un modello di preferenza più realistico, è necessario raccogliere dati basati su \textbf{\textcolor{BurntOrange}{scelte effettive}} tra un insieme di alternative, includendo anche l’opzione di non acquisto.
\\ \hspace*{1.5em}
Si possono distinguere due tipologie di dati.
Nel caso dei \emph{revealed preference data}, si osservano direttamente le scelte compiute sul mercato.
Nel caso dei \emph{stated preference data}, si analizza invece la reazione di un panel di consumatori a un insieme di stimoli appositamente costruiti, corrispondenti ad alternative ipotetiche.
\\ \hspace*{1.5em}
La \textbf{choice-based conjoint analysis} si fonda sui dati di preferenza dichiarata, poiché offre un’elevata flessibilità nella generazione degli stimoli e nella raccolta dei dati, a fronte di una possibile riduzione del realismo.
Un ruolo cruciale è svolto dalla progettazione accurata degli esperimenti, come nel caso dei disegni ortogonali, al fine di massimizzare l’informazione raccolta senza imporre un eccessivo carico cognitivo all’utente, evitando un numero troppo elevato di alternative per ciascuna scelta o un numero eccessivo di scelte complessive.
\\ \hspace*{1.5em}
Oltre alle problematiche classiche della progettazione sperimentale, è importante selezionare, ed eventualmente combinare, attributi e livelli, escludere combinazioni prive di significato, evitare profili incoerenti, come prestazioni migliori associate a un prezzo inferiore, e gestire correttamente le variabili ordinali.
\\ \hspace*{1.5em}
Un modello tipicamente stimato nell’ambito della choice-based conjoint analysis è il \textbf{\textcolor{BurntOrange}{multinomial logit (MNL)}}.
Versioni più sofisticate della CBC si basano su modelli Bayesiani gerarchici, che consentono di tenere conto dell’eterogeneità dei consumatori.
\\ \hspace*{1.5em}
Attraverso la stima delle utilità di partworth, è possibile prevedere la quota di mercato di prodotti non ancora offerti e valutare l’impatto delle decisioni di pricing.

\paragraph{Microfondamenti dei modelli MNL: utilità casuali}
Si assuma che il consumatore scelga una tra $n$ alternative sulla base dell’utilità associata a ciascun bene.
L’utilità dell’alternativa $i \in [n]$ è data da
\(
V_i + \varepsilon_i,
\)
ossia dalla somma di una componente deterministica e di una componente casuale.
La componente deterministica è spesso indicata come \textbf{utilità rappresentativa}.
\\ \hspace*{1.5em}
L’alternativa $i$ viene selezionata se
\(
V_i + \varepsilon_i > V_j + \varepsilon_j,
\qquad
\forall j \neq i.
\)
Ne segue che la probabilità di selezione dell’alternativa $i$ è
\[
\pi_i
=
\mathbb{P}\{ V_i + \tilde{\varepsilon}_i > V_j + \tilde{\varepsilon}_j,\ \forall j \neq i \}
=
\mathbb{P}\{ \varepsilon_j < \varepsilon_i + V_i - V_j,\ \forall j \neq i \}.
\]
\hspace*{1.5em}
Assumendo indipendenza tra le componenti casuali e condizionando rispetto a $\varepsilon_i$, si ottiene
\[
\pi_i \mid \varepsilon_i
=
\prod_{j \neq i}
F_j(\varepsilon_i + V_i - V_j),
\]
dove $F_j$ è la funzione di distribuzione di $\varepsilon_j$.
Integrando rispetto alla densità $f_i$ di $\varepsilon_i$, si ricava
\[
\pi_i
=
\int_{-\infty}^{+\infty}
\prod_{j \neq i}
F_j(\varepsilon_i + V_i - V_j)\, f_i(s)\, ds.
\]
\hspace*{1.5em}
Si assuma ora che tutte le componenti casuali siano indipendenti e identicamente distribuite secondo una \textbf{\textcolor{BurntOrange}{distribuzione di Gumbel}}, la cui funzione di distribuzione e densità sono date da
\[
...
\]
\hspace*{1.5em}
Osservando che per $j=i$ risulta $s+V_i-V_j=s$, è possibile includere il termine
$\exp(-e^{-s})$ all’interno del prodotto e raccogliere i termini indipendenti da $j$, ottenendo
\[
\begin{aligned}
\pi_i
&=
...
\]
\hspace*{1.5em}
Si applica ora il cambio di variabile $t = e^{-s}$, da cui $-e^{-s}ds = dt$, con estremi di integrazione pari a $+\infty$ e $0$, rispettivamente.
Tenendo conto del cambio di segno, si ottiene
\[
\pi_i
=
\int_{0}^{+\infty}
\exp\!\left(
- t \sum_{j=1}^{n} e^{-(V_i-V_j)}
\right) dt
=
...
\]
che coincide con il \textbf{\textcolor{BurntOrange}{modello multinomial logit (MNL)}}.
\\ \hspace*{1.5em}
Il modello MNL è ampiamente utilizzato nel contesto del machine learning e dei problemi di classificazione ed è relativamente semplice da trattare all’interno di modelli di ottimizzazione, ad esempio nei problemi di scelta ottima dell’assortimento.
Tuttavia, esso presenta limitazioni ben note, in particolare l’ipotesi di \textbf{indipendenza dalle alternative irrilevanti (IIA)}.
Considerando i rapporti di probabilità di scelta tra le alternative $i$ e $k$, si ha
\[
\frac{\pi_i}{\pi_k}
=
\frac{e^{V_i}/\sum_j e^{V_j}}{e^{V_k}/\sum_j e^{V_j}}
=
e^{V_i-V_k},
\]
che non dipende dalla presenza di altre alternative.
Di conseguenza, i rapporti di probabilità relativi rimangono invariati anche quando nuove alternative vengono incluse nel set di scelta.

\paragraph{Supplemento tecnico: EVT e massimi a blocchi}
Nell’ambito della \textbf{Extreme Value Theory (EVT)} si studia, tra le altre cose, la distribuzione dei massimi a blocchi.
Si consideri un campione i.i.d.\ di dimensione $n$ estratto da una distribuzione con funzione di distribuzione $F_X(x)$.
Ci si chiede quale sia la distribuzione di
\(
M_n = \max\{X_1,\ldots,X_n\}
\)
e quale sia il suo comportamento asintotico per $n \to +\infty$.
\\ \hspace*{1.5em}
In virtù dell’indipendenza, si ha
\[
F_{\max}(x)
=
\mathbb{P}\{M_n \le x\}
...
\]
\\ \hspace*{1.5em}
Nel caso di una variabile casuale uniforme $U \sim \mathcal{U}(0,1)$, risulta
\[
F_U(x) = x
\quad \Rightarrow \quad
F_{\max}(x) = x^n,
...
x \in [0,1].
\]
Per $n \to +\infty$, la distribuzione converge a una distribuzione degenere, con massa di probabilità concentrata in $x=1$.
Si osserva inoltre che in questo caso vale
\(
\mathbb{E}[M_n] = \frac{n}{n+1}.
\)
\\ \hspace*{1.5em}
Un caso degenere di questo tipo, dovuto al supporto limitato della distribuzione, risulta poco interessante.
Se si considera invece una variabile casuale con supporto illimitato superiormente, la distribuzione dei massimi tende a traslare verso $+\infty$, rendendo necessario un opportuno procedimento di normalizzazione.
\\ \hspace*{1.5em}
Nel caso della somma
\(
S_n = \sum_{k=1}^{n} X_k,
\)
rilevante per il Teorema del Limite Centrale, si introduce una normalizzazione mediante opportune successioni $a_n$ e $b_n$, tali che
\[
\lim_{n\to+\infty}
\mathbb{P}\!\left\{
\frac{S_n - a_n}{b_n} \le x
\right\}
=
\Phi(x),
\qquad
a_n = n\,\mathbb{E}(X_1),
\quad
b_n = \sqrt{n\,\mathrm{Var}(X_1)}.
\]
\\ \hspace*{1.5em}
In modo analogo, per ottenere una distribuzione limite dei massimi a blocchi, si introducono successioni di normalizzazione $d_n$ e $c_n$ e si verifica se
\[
\lim_{n\to+\infty}
\mathbb{P}\!\left\{
\frac{M_n - d_n}{c_n} \le x
\right\}
=
\lim_{n\to+\infty}
F_X^n(c_n x + d_n)
...
H(x),
\]
per una qualche funzione di distribuzione non degenere $H(x)$.
\\ \hspace*{1.5em}
\textbf{Definizione (distribuzione GEV).}
La funzione di distribuzione della distribuzione \textbf{generalized extreme value (GEV)} standard è data da
\[
H_{\xi}(x)
=
\begin{cases}
...
\]
dove $1+\xi x > 0$.
Introducendo un parametro di posizione $\mu \in \mathbb{R}$ e un parametro di scala $\sigma>0$, si ottiene la famiglia parametrizzata
\[
H_{\xi,\mu,\sigma}(x)
=
H_{\xi}\!\left(\frac{x-\mu}{\sigma}\right).
\]
Il parametro $\xi$ determina la forma della famiglia di distribuzioni.
\\ \hspace*{1.5em}
Si ricorda che due variabili casuali $V$ e $W$ differiscono solo per un parametro di scala $a>0$ e un parametro di posizione $b\in\mathbb{R}$, ossia
\(
V \stackrel{d}{=} aW + b,
\)
se e solo se appartengono allo stesso tipo di distribuzione.
\\ \hspace*{1.5em}
Per valori del parametro di forma $\xi$ diversi, si ottengono le seguenti famiglie:
se $\xi>0$ si ha una distribuzione di Fréchet, se $\xi=0$ una distribuzione di Gumbel, mentre se $\xi<0$ una distribuzione di Weibull.
La distribuzione di Weibull presenta un estremo destro finito, mentre la distribuzione di Fréchet è caratterizzata da una decrescenza più lenta rispetto alla Gumbel.
\\ \hspace*{1.5em}
Si osserva infine che la distribuzione di Gumbel riveste un ruolo fondamentale anche nei modelli di scelta discreta.
Se la condizione precedente è soddisfatta per una distribuzione limite non degenere $H$, allora si dice che la funzione di distribuzione $F$ appartiene al \textbf{dominio di attrazione massimo} di $H$, e si scrive $F \in \mathrm{MDA}(H)$.
Nel caso della distribuzione esponenziale, vale in particolare $F \in \mathrm{MDA}(H_0)$.

\paragraph{Teorema (Fisher--Tippett--Gnedenko)}
Se $F \in \mathrm{MDA}(H)$ per una distribuzione non degenere $H$, allora $H$ deve appartenere alla famiglia GEV, ossia $H = H_{\xi}$ per qualche valore di $\xi$.
...
Le politiche randomizzate risultano inoltre necessarie nel contesto del trade-off tra esplorazione e exploitation nel reinforcement learning.
...
\paragraph{Recap}
La \textcolor{DeepPink}{\textbf{programmazione dinamica}} fornisce un principio generale di \textbf{modellazione} per problemi \textbf{decisionali multistadio}, fondato sulla \textcolor{DeepPink}{\textbf{decomposizione}} del problema complessivo in una sequenza di sottoproblemi a singolo stadio e sull’uso di una \textcolor{DeepPink}{\textbf{value function}}. Tale funzione consente di bilanciare in modo sistematico la \textbf{ricompensa immediata} con il \textbf{valore atteso delle decisioni future}, attraverso una ricorsione che incorpora \textbf{azioni ammissibili}, \textcolor{DeepPink}{\textbf{fattori di rischio}} e \textbf{dinamiche di transizione} dello stato.
\\ \hspace*{1.5em}
Nel caso dei \textcolor{DeepPink}{\textbf{processi decisionali di Markov finiti}}, caratterizzati da spazi di stato e di azione discreti, la dinamica è descritta tramite \textcolor{DeepPink}{\textbf{probabilità di transizione}} dipendenti dall’azione e la ricorsione di DP assume una forma esplicita e computazionalmente ben definita. Tuttavia, l’applicazione pratica è spesso limitata dalla \textcolor{DeepPink}{\textbf{curse of modeling}} e dalla rapida crescita dimensionale dello spazio degli stati.
\\ \hspace*{1.5em}
Per superare tali difficoltà, si introduce la formulazione in termini di \textcolor{DeepPink}{\textbf{Q-factors}} e l’impiego di tecniche di \textbf{campionamento Monte Carlo}, che consentono di riscrivere la ricorsione di programmazione dinamica ottenendo uno \textcolor{DeepPink}{\textbf{scambio tra attesa e ottimizzazione}}. Questa riformulazione risulta particolarmente utile nei problemi di grande scala o con dinamica sconosciuta, aprendo la strada a metodi di apprendimento \textbf{model-free} e all’utilizzo di \textbf{architetture di approssimazione}.
\\ \hspace*{1.5em}
Un’idea strettamente collegata è l’introduzione dello \textcolor{DeepPink}{\textbf{stato post-decisione}}, che consente di separare la componente deterministica della decisione dall’incertezza legata ai fattori di rischio, riformulando la ricorsione di programmazione dinamica in una forma caratterizzata da \textbf{ottimizzazione deterministica} e \textbf{attesa esterna}. Questa struttura semplifica sia l’analisi teorica sia lo sviluppo di algoritmi numerici.
\\ \hspace*{1.5em}
Questi principi vengono illustrati attraverso esempi rilevanti, quali l’\textbf{aumento dello stato} nei problemi di gestione delle scorte in presenza di \textbf{lead time} o articoli deperibili, e i modelli di \textcolor{DeepPink}{\textbf{revenue management}}. In quest’ultimo contesto, nelle versioni statiche e dinamiche, con \textbf{segmentazione perfetta} o con \textbf{scelta del cliente}, emergono naturalmente \textbf{livelli di protezione}, politiche dipendenti dal tempo e diverse forme della ricorsione di DP, determinate dalla struttura informativa e dal timing delle decisioni.
...
\paragraph{Variabili di stato post-decisione}
Una \emph{forma alternativa} è
\[
V_t(s_t)= \mathbb{E}_t\!\left[\operatorname{opt}_{x_t\in X(s_t)}\left\{f_t(x_t,s_t)+\gamma V_{t+1}(s_{t+1})\right\}\right].
\]
\hspace*{1.5em}
Il \textbf{problema di ottimizzazione} interno è deterministico e l'attesa esterna può essere stimata tramite campionamento statistico.
Poiché si tratta di vantaggi rilevanti, si può tentare di \textbf{riformulare la ricorsione standard di programmazione dinamica} in questa forma, anche quando essa non rappresenta la formulazione più naturale del problema. In alcuni casi, ciò può essere ottenuto riscrivendo la \textbf{dinamica del sistema} tramite l’introduzione di \textbf{variabili di stato post-decisione}.
Nella rappresentazione consueta della dinamica di un sistema, si considerano transizioni dallo stato \(s_t\) allo stato \(s_{t+1}\), che dipendono dalla decisione selezionata \(x_t\) e dalla realizzazione del \textbf{fattore di rischio} \(\xi_{t+1}\). In alcune situazioni, risulta tuttavia conveniente introdurre uno \textbf{stato intermedio}, osservato dopo che la decisione \(x_t\) è stata presa ma prima che il fattore di rischio \(\xi_{t+1}\) si realizzi. Tale stato prende il nome di \textcolor{DeepPink}{\textbf{stato post-decisione}} e può essere indicato con \(s_t^x\).

\paragraph{Esempio}
Dall’esempio si evince che l’introduzione dello \textbf{stato post-decisione} permette di separare la componente deterministica della decisione da quella stocastica legata al fattore di rischio. In questo modo, l’incertezza viene assorbita in un’\textbf{attesa priva di decisioni}, mentre l’ottimizzazione diventa \textbf{deterministica}. La ricorsione di programmazione dinamica assume quindi una forma con \textbf{scambio tra attesa e ottimizzazione}, che facilita sia l’analisi sia l’approssimazione numerica.

\paragraph{Relazione tra funzioni valore}
Si definisce il valore del post-decision state $V_t(s_t)$ e $V_t^x(s_t^x)$: $V_t^x(s_t^x)= \mathbb{E}\!\left[\,V_{t+1}(s_{t+1}) \mid s_t^x\right]$. Quindi la \textbf{ricorsione standard} può essere riscritta come \textbf{ottimizzazione deterministica}
\[
V_t(s_t)= \operatorname{opt}_{x_t\in X(s_t)}\left\{f_t(s_t,x_t)+\gamma V_t^x(s_t^x)\right\}.
\]
\hspace*{1.5em}
Scrivendo la relazione un passo indietro e sostituendo l'espressione precedente, si ottiene una ricorsione che presenta nuovamente lo \textcolor{DeepPink}{\textbf{scambio tra attesa e ottimizzazione}}
\[
V_{t-1}^x(s_{t-1}^x)
= \mathbb{E}\left[ V_t(s_t) \Bigm| s_{t-1}^x \right] =
\mathbb{E}\!\left[
...
\]
\hspace*{1.5em}
Trascurando lo slittamento temporale privo di conseguenze, si ottiene nuovamente una ricorsione di programmazione dinamica caratterizzata dallo \textbf{scambio tra attesa e ottimizzazione}. La ricorsione di DP formulata in termini di \textbf{Q-factors} si inserisce naturalmente in questo quadro, poiché la coppia \textbf{stato--azione} \((i,a)\) può essere interpretata come uno \textbf{stato post-decisione}, prima dell’osservazione della transizione casuale verso il nuovo stato.

\paragraph{Aumento dello stato nella gestione delle scorte}
Nel contesto della gestione delle scorte, l’inventario \textbf{on-hand} appare come la variabile di stato naturale e come la base più immediata per qualsiasi decisione di riordino. Al contrario, le decisioni di ordinazione non dovrebbero essere basate sull’inventario on-hand, bensì sull’\textbf{inventario disponibile}, che tiene conto sia dell’inventario \textbf{on-order} (articoli ordinati al fornitore ma non ancora consegnati) sia dell’eventuale \textbf{backlog}.
\\ \hspace*{1.5em}
L’equazione di stato standard
\(
I_{t+1} = I_t + x_t - d_{t+1},
\)
assume che quanto ordinato all’istante di tempo \(t\) sia immediatamente disponibile per soddisfare la domanda durante l’intervallo di tempo successivo \(t+1\).
\\ \hspace*{1.5em}
Se il \textbf{lead time} di consegna è un numero intero \(LT \ge 1\) di intervalli di tempo, è necessario introdurre variabili di stato che tengano traccia di ciò che è stato ordinato in passato e si trova ancora nella pipeline di trasporto.
\\ \hspace*{1.5em}
Si denotino tali variabili di stato con \(z_{t,\tau}\), che rappresentano la quantità che verrà consegnata \(\tau\) intervalli di tempo dopo l’istante corrente \(t\), dove \(\tau = 0,1,2,\dots,LT-1\). In particolare, \(z_{t,0}\) rappresenta ciò che è immediatamente disponibile all’istante di tempo \(t\) ed è coinvolto nell’equazione di transizione dell’inventario on-hand:
\(
I_{t+1} = I_t + z_{t,0} - d_{t+1},
\)
se si trascura l’incertezza sulla domanda.
\\ \hspace*{1.5em}
...
z_{t+1,LT-1} = x_t.
\)
\hspace*{1.5em}L’equazione di transizione generale per le variabili di stato aggiuntive \(z_{t,\tau}\), per \(\tau < LT-1\), si riduce a un semplice \textcolor{DeepPink}{\textbf{shift temporale}}
\[
z_{t+1,\tau} = z_{t,\tau+1},
\qquad \tau = 0,1,\dots,LT-2.
\]
\hspace*{1.5em}
In modo analogo, nel caso di articoli \textbf{deperibili} è necessario descrivere l’inventario a ciascuna età, introducendo un array di variabili di stato \(I_{t,\tau}\) che rappresentano la quantità di inventario on-hand al tempo \(t\) con un’età pari a \(\tau\) periodi di tempo.
Le transizioni di stato non sono immediate da scrivere in forma esplicita e dipendono dal tipo di politica di issuing adottata, \textbf{FIFO} (\emph{first-in-first-out}) oppure \textbf{LIFO} (\emph{last-in-first-out}).

\paragraph{Revenue management}
Il \textcolor{DeepPink}{\textbf{revenue management}} consiste in una serie di modelli e tecniche finalizzate a massimizzare il \textbf{ricavo} ottenuto dalla vendita di \textbf{risorse deperibili}. L’\emph{idea} è stata introdotta, con il nome di \textcolor{DeepPink}{\emph{yield management}}, nell’industria del trasporto aereo.
\\ \hspace*{1.5em}
Si distinguono due approcci fondamentali al revenue management: l’approccio \textbf{quantity-based} e l’approccio \textbf{price-based}. Nel primo caso, la disponibilità della risorsa viene limitata secondo una certa politica al fine di massimizzare il ricavo; nel secondo caso, i prezzi vengono aggiustati dinamicamente e si parla di \textbf{dynamic pricing}.
\\ \hspace*{1.5em}
...
\\ \hspace*{1.5em}
Si considerino \(C\) unità di una singola risorsa, ad esempio posti su un aeromobile, che devono essere allocate a \(n\) classi tariffarie, indicizzate da \(j = 1,2,\dots,n\). Le unità della classe \(j\) sono vendute al prezzo \(p_j\), con
\(
p_1 > p_2 > \dots > p_n,
\)
così che la classe 1 corrisponde alla classe dalla quale si ottiene il ricavo più elevato.
\\ \hspace*{1.5em}
Si assume che i posti allocati alle diverse classi siano fisicamente \textbf{identici}, ma vengano differenziati mediante \textbf{servizi accessori} (\emph{ancillaries}), come diritti di cancellazione, restrizioni sul weekend o servizi a bordo, così da offrire pacchetti differenti.
\\ \hspace*{1.5em}
Il prezzo di ciascuna classe è fissato, mentre il controllo avviene sulla \textbf{disponibilità di inventario}, limitando il numero di posti offerti a ciascuna classe. La domanda \(D_j\) di ciascuna classe è aleatoria e può realizzarsi secondo schemi differenti, dando luogo a formulazioni di problema diverse.
\\ \hspace*{1.5em}
Due sono le caratteristiche essenziali considerate. La prima riguarda il \textcolor{DeepPink}{\textbf{comportamento dei clienti}}: in un mercato perfettamente segmentato, ciascun passeggero è disposto ad acquistare una sola classe e non è necessario modellare le preferenze; al contrario, per tener conto delle preferenze è necessario introdurre un \textbf{modello di scelta}. La seconda riguarda il \textcolor{DeepPink}{\textbf{timing della domanda}}: sarebbe auspicabile che la domanda si realizzasse in modo sequenziale, dalla classe 1 fino alla classe \(n\), mentre il caso peggiore si verifica quando i clienti a basso budget arrivano per primi.
\\ \hspace*{1.5em}
Un modello in cui esistono intervalli di tempo disgiunti, durante i quali una sola classe è in domanda, è detto \textbf{statico}; il termine \textbf{dinamico} è invece riservato a modelli in cui le richieste dei clienti per classi diverse sono intercalate nel tempo. La politica decisionale può essere espressa in termini di \textbf{livelli di protezione}, ossia il numero massimo di posti disponibili per ciascuna classe, oppure in termini di \textbf{bid-prices}, ovvero il prezzo minimo al quale si è disposti a vendere un posto.

\subsection{Modello statico con segmentazione perfetta della domanda}

\paragraph{Ipotesi di segmentazione e struttura della domanda}
Nel modello statico con \textcolor{DeepPink}{\textbf{segmentazione perfetta della domanda}}, le richieste associate alle diverse classi tariffarie sono assunte \textbf{indipendenti} e si realizzano in modo \textbf{sequenziale}, a partire dalla classe a prezzo più basso. Nessun cliente è disposto a passare a una classe differente, il che implica l’assenza di sostituzione tra prodotti e rende superflua la modellazione delle preferenze individuali.

\paragraph{Stadi decisionali e variabile di stato}
In questo contesto si associano gli \textbf{stadi decisionali} alle classi tariffarie, indicizzate in ordine decrescente \( j = n, n-1, \dots, 1 \). La variabile di stato naturale è \( s_j \), che rappresenta la \textbf{capacità residua} all’inizio dello stadio \( j \); lo stato iniziale è quindi \( s_n = C \), pari al numero totale di posti disponibili.

\paragraph{Funzione valore e condizioni al contorno}
L’obiettivo complessivo è determinare il \textbf{ricavo atteso massimo} ottenibile allocando i \( C \) posti alle \( n \) classi, valore indicato con \( V_n(C) \), mentre la condizione al contorno è data da \( V_0(s_0)=0 \) per ogni \( s_0=0,1,\dots,C \). Non si considerano gruppi o famiglie e, data l’indipendenza, non emerge alcun effetto di apprendimento della domanda lungo il processo.

\paragraph{Sequenza degli eventi e formulazione della ricorsione}
...
\paragraph{Indipendenza della decisione dalla distribuzione della domanda}
Sebbene ciò sembri in contrasto con il fatto che le decisioni dovrebbero essere prese prima di osservare la domanda, si dimostra che la decisione ottima non dipende dalla distribuzione di probabilità di \( D_j \).

\paragraph{Interpretazione operativa della decisione}
L’intuizione è che non sia necessario fissare a priori il valore di \( x_j \). Le richieste possono essere osservate una alla volta e, per ciascuna di esse, si decide se accettarla o rifiutarla; nel momento in cui una richiesta viene rifiutata, la classe viene dichiarata chiusa.
\\ \hspace*{1.5em}
In questo modo, la ricorsione di programmazione dinamica assume la \textcolor{DeepPink}{\textbf{forma scambiata}}
\[
V_j(s_j)=\mathbb{E}\!\left[\max_{0\le x_j\le \min\{s_j,D_j\}}\bigl(p_j x_j+V_{j-1}(s_j-x_j)\bigr)\right].
\]

\paragraph{Shift di indice e semplificazione della notazione}
Per semplificare la notazione, si introduce uno \textbf{shift inconsequenziale dell’indice} e si omettono i pedici di stadio nelle variabili di stato e di decisione, ottenendo
\[
V_{j+1}(s)=\mathbb{E}\!\left[\max_{0\le x\le \min\{s,D_{j+1}\}}\bigl(p_{j+1}x+V_j(s-x)\bigr)\right].
\]
\\ \hspace*{1.5em}
...
Per analizzare la struttura della politica ottima si introduce il \textcolor{DeepPink}{\textbf{valore marginale atteso della capacità}} \( \Delta V_j(s):=V_j(s)-V_j(s-1) \), che misura il \textbf{costo opportunità} di un posto aereo dato un livello di capacità residua \( s \).
\\ \hspace*{1.5em}
Utilizzando una \textbf{somma telescopica}, la ricorsione può essere riscritta come
\[
V_{j+1}(s)=V_j(s)+\mathbb{E}\!\left[\max_{0\le x\le \min\{s,D_{j+1}\}}\sum_{z=1}^{x}\bigl(p_{j+1}-\Delta V_j(s+1-z)\bigr)\right].
\]


La riscrittura precedente si basa su un \textbf{trucco standard del mestiere}, ossia su una \textcolor{DeepPink}{\textbf{somma telescopica}}
\[
\begin{aligned}
V_j(s-x)
= V_j(s)-\bigl[V_j(s)-V_j(s-x)\bigr] 
= V_j(s)-\bigl[V_j(s)\pm V_j(s-1)\pm \dots \pm V_j(s+1-x)-V_j(s-x)\bigr] \\
= V_j(s)-\sum_{z=1}^{x}\bigl[V_j(s+1-z)-V_j(s-z)\bigr] 
= V_j(s)-\sum_{z=1}^{x}\Delta V_j(s+1-z).
\end{aligned}
\]

\paragraph{Proprietà di monotonicità}
Si può dimostrare che valgono \( \Delta V_j(s+1)\le \Delta V_j(s) \) e \( \Delta V_{j+1}(s)\ge \Delta V_j(s) \), da cui segue che il valore della capacità decresce con la capacità disponibile e cresce con il numero di stadi rimanenti.
\\ \hspace*{1.5em}
Di conseguenza, i termini \( p_{j+1}-\Delta V_j(s+1-z) \) sono decrescenti in \( z \), e la soluzione ottima si ottiene aumentando \( x \) finché compare il primo termine negativo oppure si raggiunge \( \min\{s,D_{j+1}\} \).
Questo porta alla definizione di un \textcolor{DeepPink}{\textbf{livello di protezione annidato ottimo}}
\[
y_j^\ast:=\max\{y:\;p_{j+1}<\Delta V_j(y)\}, \quad j=1,\dots,n-1.
\]

\paragraph{Decisione ottima e interpretazione}
Il livello \( y_j^\ast \) vincola il numero massimo di biglietti vendibili per la classe \( j+1 \), con decisione ottima
\(
x_{j+1}^\ast(s_{j+1},D_{j+1})=\min\{(s_{j+1}-y_j^\ast)^+,\,D_{j+1}\}.
\)
\\ \hspace*{1.5em}
Le richieste possono essere soddisfatte progressivamente senza conoscere \( D_{j+1} \) in anticipo, il che giustifica lo \textcolor{DeepPink}{\textbf{scambio tra attesa e ottimizzazione}} nella ricorsione di programmazione dinamica.


\paragraph{Modello dinamico con segmentazione perfetta della domanda}
Si rilassa l’ipotesi secondo cui la domanda delle diverse classi si realizzi in modo sequenziale su intervalli di tempo disgiunti; rimane tuttavia valida l’assunzione di \textcolor{DeepPink}{\textbf{segmentazione rigida del mercato}}. Il tempo entra esplicitamente nella ricorsione di programmazione dinamica, ma si ottiene un modello semplice assumendo che gli intervalli temporali siano sufficientemente piccoli da consentire l’osservazione di \textbf{al più un arrivo per intervallo}.
\\ \hspace*{1.5em}
Si denoti con \( \lambda_j(t) \) la probabilità di un arrivo della classe \( j \) nell’intervallo di tempo \( t \), per \( t=1,\dots,T \). Poiché il mercato è perfettamente segmentato, tali probabilità si riferiscono a eventi indipendenti e devono soddisfare il vincolo di consistenza \( \sum_{j=1}^n \lambda_j(t)\le 1 \) per ogni \( t \).
\\ \hspace*{1.5em}
La ricorsione DP mira a determinare le funzioni valore \( V_t(s) \), dove \( s \) rappresenta la \textbf{capacità residua}, soggette alle condizioni al contorno \( V_t(0)=0 \) per \( t=1,\dots,T \) e \( V_{T+1}(s)=0 \) per \( s=0,1,\dots,C \), dove \( T+1 \) indica la fine dell’orizzonte temporale.
\\ \hspace*{1.5em}
Si introduca la variabile casuale \( R(t) \), che rappresenta il \textbf{ricavo disponibile} al tempo \( t \), pari a \( p_j \) quando si verifica un arrivo della classe \( j \) e pari a zero in caso contrario. In presenza di un arrivo, si deve decidere se accettare o meno la richiesta tramite una variabile decisionale binaria \( x\in\{0,1\} \).
\\ \hspace*{1.5em}
Come nel modello statico, non è necessario prendere decisioni in anticipo, poiché è sufficiente reagire alla richiesta osservata; di conseguenza, la ricorsione di programmazione dinamica assume nuovamente la \textcolor{DeepPink}{\textbf{forma scambiata}}
\[
V_t(s)=\mathbb{E}\!\left[\max_{x\in\{0,1\}}\bigl(R(t)x+V_{t+1}(s-x)\bigr)\right].
\]
\\ \hspace*{1.5em}
Utilizzando ancora i \textbf{valori marginali attesi della capacità}, è possibile definire una politica ottima in termini di \textcolor{DeepPink}{\textbf{livelli di protezione}}, che in questo contesto risultano però \textbf{dipendenti dal tempo}.

\paragraph{Modello dinamico con scelta del cliente}
Nel caso in cui si rilassi l’ipotesi di \textcolor{DeepPink}{\textbf{mercato perfettamente segmentato}}, è necessario introdurre un esplicito \textcolor{DeepPink}{\textbf{modello di scelta del cliente}}. Un approccio consiste nel suddividere il mercato in segmenti e stimare, per ciascun segmento, sia la frazione di passeggeri sia la probabilità che una determinata classe venga acquistata quando essa è offerta.
\\ \hspace*{1.5em}
La \textcolor{DeepPink}{\textbf{decisione di controllo}} all’istante di tempo \(t\) non è più una quantità, ma un insieme: il sottoinsieme \(S_t \subseteq \mathcal{N}=\{1,\dots,n\}\) delle classi che vengono offerte. A ciascuna classe \(j\) è associato un ricavo \(p_j\); si introduce inoltre \(p_0=0\) per rappresentare il caso di \textbf{non-acquisto}, ossia quando un passeggero è disposto a volare ma non sceglie nessuna delle classi offerte.
\\ \hspace*{1.5em}
Formalmente, una potenziale richiesta arriva al tempo \(t\) con probabilità \(\lambda\) e, dato l’insieme \(S_t\), il cliente sceglie una classe \(j\in S_t\cup\{0\}\) con probabilità \(P_j(S_t)\), con vincoli \(P_j(S)\ge 0\) e \(\sum_{j\in S}P_j(S)+P_0(S)=1\). Ne segue che le probabilità effettive di acquisto al tempo \(t\) sono \(\lambda P_j(S_t)\) per \(j=1,\dots,n\) e \((1-\lambda)+\lambda P_0(S_t)\) per il non-acquisto.
\\ \hspace*{1.5em}
In questo contesto, la \textcolor{DeepPink}{\textbf{ricorsione di programmazione dinamica}} assume la forma
\[
V_t(s)=\max_{S_t\subseteq\mathcal{N}}\left\{\sum_{j\in S_t}\lambda P_j(S_t)\big(p_j+V_{t+1}(s-1)\big)+\big(\lambda P_0(S_t)+1-\lambda\big)V_{t+1}(s)\right\},
\]
poiché l’offerta \(S_t\) deve essere fissata \emph{prima} della decisione del passeggero. Di conseguenza, a differenza dei modelli con segmentazione perfetta, l’ottimizzazione avviene nella forma usuale, come \textcolor{DeepPink}{\textbf{massimizzazione di un valore atteso}}.
...
\paragraph{Recap}
La \textcolor{MediumVioletRed}{\textbf{programmazione dinamica numerica per stati discreti}} riguarda problemi decisionali in cui lo stato evolve a tempo discreto $t=0,1,2,\ldots$ su uno spazio di stati finito o numerabile e la dinamica è descritta da \textbf{probabilità di transizione} che rispettano la proprietà di Markov. Nel caso \textbf{omogeneo} le probabilità non dipendono dal tempo, ipotesi naturale nei problemi a \textbf{orizzonte infinito}; nei \textbf{Markov Decision Processes} (MDP) la dinamica è \textbf{parzialmente controllata} tramite la scelta di un’azione $a\in\mathcal A(i)$ e, per ogni azione, la transizione è descritta da $\pi(i,a,j)$, che consente di rappresentare il modello anche tramite insiemi di matrici di transizione indicizzate dall’azione.  
\\ \hspace*{1.5em}
La valutazione delle prestazioni di una politica richiede di distinguere le proprietà della catena indotta: la presenza di \textbf{stati assorbenti} genera stati transienti e può rendere la dinamica dipendente dalla condizione iniziale; se la catena non è \textbf{irriducibile} si possono ottenere limiti diversi a seconda dello stato di partenza, mentre l’assenza di \textbf{aperiodicità} impedisce di definire una distribuzione stazionaria. Per evitare comportamenti degeneri e garantire che gli algoritmi “vedano” gli stati con sufficiente frequenza, si assume che il processo sia \textbf{ben comportato} per qualunque politica, tipicamente con struttura \textbf{unichain} e senza periodicità, così da rendere sensata l’idea di prestazione di lungo periodo e l’uso di metodi iterativi.  
\\ \hspace*{1.5em}
Nei problemi a \textbf{orizzonte temporale finito} la soluzione è espressa da una ricorsione di Bellman in cui la funzione di valore $V_t(i)$ dipende esplicitamente da $t$ e bilancia contributo immediato e valore futuro atteso,
\( V_t(i)=\opt_{a\in\mathcal A(i)}\{f(i,a)+\gamma\sum_j \pi(i,a,j)V_{t+1}(j)\} \),
con condizione terminale al bordo che determina il comportamento all’ultimo istante decisionale. Nell’esempio di \textbf{arresto ottimo} su catena lineare, l’azione \emph{wait} induce un \emph{random walk} mentre l’azione di stop produce una ricompensa $R_i$ e porta a uno stato assorbente; la struttura locale delle transizioni implica una \textbf{matrice di transizione tridiagonale} e consente una forma vettoriale compatta del confronto tra ricompensa e valore di continuazione, evidenziando che la politica ottima può essere \textbf{non stazionaria} e dipendere sia dallo stato sia dal tempo residuo.  
\\ \hspace*{1.5em}
Nei problemi a \textbf{orizzonte infinito} la funzione di valore è invece definita come soluzione di un’equazione di punto fisso del tipo
\( V(i)=\opt_{a\in\mathcal A(i)}\{f(i,a)+\gamma\sum_j \pi(i,a,j)V(j)\} \),
o in forma equivalente tramite contributi immediati $h(i,a,j)$ dentro la somma. Per MDP finiti con \textbf{sconto stretto} $\gamma<1$ e contributi immediati limitati, l’operatore di Bellman $\mathcal T$ e l’operatore associato a una politica fissata $\mathcal T_\mu$ risultano \textbf{contrazioni}, quindi ammettono un unico punto fisso: \( \mathbf V=\mathcal T\mathbf V \) caratterizza il valore ottimo e \( \mathbf V_\mu=\mathcal T_\mu \mathbf V_\mu \) caratterizza il valore della politica.  
\\ \hspace*{1.5em}
Su questa base si introducono due schemi numerici: la \textcolor{MediumVioletRed}{\textbf{value iteration}}, che applica ripetutamente $\mathcal T$ ottenendo aggiornamenti poco costosi e una convergenza al limite verso $\mathbf V$, e la \textcolor{MediumVioletRed}{\textbf{policy iteration}}, che alterna \emph{policy evaluation} e \emph{policy improvement}. In particolare, fissata $\mu$, la valutazione della politica equivale a risolvere un sistema lineare
\( (\mathbf I-\gamma\Pi_\mu)\mathbf V_\mu=f_\mu \),
mentre il miglioramento sceglie per ogni stato un’azione in
\( \tilde\mu(i)\in\arg\opt_{a\in\mathcal A(i)}\{f(i,a)+\gamma\sum_j \pi(i,a,j)V_\mu(j)\} \),
garantendo che la politica non peggiori e portando a convergenza in un numero finito di passi perché le politiche stazionarie deterministiche sono in numero finito.  
\\ \hspace*{1.5em}
Il confronto tra i metodi mette in luce un continuum di approcci intermedi: quando la valutazione esatta tramite eliminazione di Gauss è impraticabile su grandi sistemi o su matrici sparse, si può valutare $\mu$ iterando il punto fisso di $\mathcal T_\mu$ e interrompere dopo un numero finito di passi ottenendo una stima $\hat V_\mu$, applicando poi il miglioramento; questo schema è alla base della \textbf{optimistic policy iteration} e, più in generale, della \textbf{generalized policy iteration}. La distinzione diventa cruciale nel \textcolor{MediumVioletRed}{\textbf{reinforcement learning}}: se il modello non è noto, si ricorre a metodi \emph{model-free} che sostituiscono $V$ con i \textbf{Q-factors} $Q(s,a)$, con approcci \emph{off-policy} come il \textbf{Q-learning} e approcci \emph{on-policy} come \textbf{SARSA}, in cui non è realistico stimare esattamente il valore di una politica e i passi di miglioramento devono essere eseguiti comunque in modo anticipato.


\paragraph{Catene di Markov a tempo discreto}
Una \textcolor{MediumVioletRed}{\textbf{catena di Markov a tempo discreto}} è un \emph{processo stocastico} caratterizzato da una variabile di stato $s_t$, con $t = 0,1,2,\ldots$, che assume valori in un \textbf{insieme discreto}. Quando lo spazio degli stati è numerabile, è possibile associare gli stati a numeri interi e rappresentare il processo mediante un \emph{grafo orientato}, in cui le transizioni sono descritte da archi diretti etichettati con le \textcolor{MediumVioletRed}{\textbf{probabilità di transizione}}. Le probabilità di transizione sono \emph{probabilità condizionate} e dipendono esclusivamente dallo stato corrente, secondo la proprietà di Markov; in particolare, nel caso \textbf{omogeneo} (invariante nel tempo), esse non dipendono dall’istante temporale, ipotesi tipica nei problemi a \emph{orizzonte infinito}.  
\\ \hspace*{1.5em}
La dinamica della catena può essere descritta raccogliendo le probabilità di transizione in una \textcolor{MediumVioletRed}{\textbf{matrice di transizione a un passo}}, in cui ogni riga rappresenta uno stato di partenza e ogni colonna uno stato di arrivo; poiché dopo ogni transizione il processo deve necessariamente trovarsi in uno degli stati dello spazio degli stati, la somma degli elementi di ciascuna riga è pari a uno. Nei \emph{Markov Decision Processes} (MDP), le transizioni sono \textbf{parzialmente controllate} dalla scelta di un’azione: a ogni stato $i$ è associato un insieme finito di azioni ammissibili, e per ciascuna azione è definita una distribuzione di probabilità di transizione sugli stati successivi.  
\\ \hspace*{1.5em}
Per valutare le prestazioni di una \textbf{politica di controllo stazionaria}, si introducono le \emph{probabilità stazionarie di lungo periodo} di trovarsi nei diversi stati, raccolte in un vettore; tali probabilità, tuttavia, possono non esistere se la catena non è sufficientemente \emph{ben comportata}. Dall’esempio si evince che, se la catena non è \textbf{irriducibile}, le probabilità di lungo periodo possono dipendere dallo stato iniziale; per questo motivo si assume una catena \textbf{unichain}, connessa, in cui ogni stato è raggiungibile da ogni altro in tempo finito con probabilità positiva ed è visitato infinitamente spesso nel lungo periodo.  
\\ \hspace*{1.5em}
Nel caso di stati \textbf{assorbenti}, una volta raggiunti essi vengono visitati per sempre, mentre gli altri stati risultano \emph{transienti}; al contrario, in una catena priva di stati assorbenti tutti gli stati possono essere \emph{ricorrenti}. In generale, la presenza simultanea di stati ricorrenti e transienti non impedisce necessariamente l’esistenza di una distribuzione stazionaria, ma può creare difficoltà per il funzionamento di alcuni algoritmi di apprendimento, che richiedono che ogni stato venga visitato con sufficiente frequenza. Infine, in presenza di \textbf{periodicità} negli stati non è possibile definire una distribuzione stazionaria; per questo motivo, le catene \textbf{aperiodiche} sono una condizione fondamentale affinché valgano diversi risultati teorici rilevanti. Nel seguito si assume che il processo di Markov considerato sia ben comportato per qualunque scelta della politica di controllo.

\paragraph{Processi decisionali di Markov a orizzonte temporale finito}
Una \textcolor{MediumVioletRed}{\textbf{catena di decisione di Markov a orizzonte finito}} è caratterizzata da una ricorsione di \textbf{programmazione dinamica} in cui, a ogni istante $t$, la funzione di valore $V_t(i)$ bilancia il \emph{contributo immediato} associato all’azione scelta nello stato $i$ e il \emph{valore atteso futuro}, calcolato tramite le probabilità di transizione e la funzione di valore allo stadio successivo, secondo la relazione
\[ V_t(i) = \opt_{a \in \mathcal A(i)} \bigl\{ f(i,a) + \gamma \sum_j \pi(i,a,j)\, V_{t+1}(j) \bigr\}; \]
la funzione di valore dipende quindi esplicitamente dal tempo e dall’orizzonte finale.
\\ \hspace*{1.5em}
Il problema viene illustrato tramite un esempio di \textbf{arresto ottimo}, in cui gli stati sono disposti lungo una catena lineare che rappresenta un \emph{random walk}: dagli stati interni è possibile rimanere nello stesso stato, salire o scendere con probabilità assegnate, mentre dagli stati di bordo sono ammesse solo alcune transizioni. A ogni istante temporale si deve scegliere se \emph{attendere}, lasciando evolvere il sistema in modo stocastico senza costi, oppure \emph{fermare il processo}.
\\ \hspace*{1.5em}
Se si sceglie di attendere, lo stato evolve secondo il meccanismo di random walk e le probabilità di transizione coincidono con quelle associate all’azione \emph{wait}, cioè $\pi_{ij} = \pi(i,\text{wait},j)$. Se invece si sceglie di fermare il processo, si ottiene una \textbf{ricompensa dipendente dallo stato} $R_i$ e il sistema entra in uno stato terminale assorbente, nel quale rimane fino all’istante finale dell’orizzonte. Questo meccanismo può essere interpretato come il problema di \emph{vendere in modo ottimale un asset}, il cui prezzo segue un random walk non influenzato dalle decisioni del decisore, mentre lo stato ha natura puramente informativa.
\\ \hspace*{1.5em}
Si osserva una leggera deviazione dalla notazione standard, poiché la decisione è consentita fino all’istante finale $T$ incluso, che rappresenta l’ultimo istante in cui è possibile fermare il processo; ciò implica che la \textbf{politica ottima non è stazionaria}. Le funzioni di valore $V_t(i)$ devono essere determinate per tutti gli istanti e tutti gli stati, imponendo come condizione al bordo
\( V_T(i) = R_i \),
cioè che, al tempo finale, la scelta ottima consista nel fermare immediatamente il processo.
\\ \hspace*{1.5em}
Le equazioni di programmazione dinamica assumono una forma esplicita che confronta, in ciascuno stato, la ricompensa immediata ottenibile fermando il processo con il valore scontato dell’attesa, che negli stati interni può essere scritto come
\( \gamma\bigl(\pi_{i,i-1}V_{t+1}(i-1)+\pi_{i,i}V_{t+1}(i)+\pi_{i,i+1}V_{t+1}(i+1)\bigr) \).
Dall’esempio si evince che la struttura delle transizioni induce una \textbf{matrice di transizione tridiagonale}, che consente una rappresentazione compatta del problema.
\\ \hspace*{1.5em}
In forma vettoriale, la ricorsione può essere scritta come
\( \mathbf V_t = \max\{\mathbf R, \gamma \Pi \mathbf V_{t+1}\} \),
dove il massimo è inteso componente per componente, evidenziando la natura deterministica dell’operatore di Bellman nel caso a orizzonte temporale finito.

\paragraph{Implementazione numerica della politica ottima}
La funzione MATLAB \textcolor{MediumVioletRed}{\texttt{\texttt{FindPolicyFiniteRW}}} implementa la \emph{ricorsione di programmazione dinamica} per un problema di \textbf{arresto ottimo} a orizzonte temporale finito, dato una matrice di transizione tridiagonale, un vettore di payoff, un orizzonte temporale e un fattore di sconto.
\\ \hspace*{1.5em}
La funzione calcola iterativamente, procedendo all’indietro nel tempo, la \textbf{funzione di valore} e la \textbf{politica ottima}, confrontando in ogni stato il valore dell’attesa con il payoff immediato.
\\ \hspace*{1.5em}
In uscita si ottengono una tabella dei valori e una tabella binaria delle decisioni, da cui si evince che la \emph{politica risultante non è stazionaria} e che le decisioni di arresto dipendono sia dallo stato sia dal tempo residuo.

\paragraph{Processi decisionali di Markov a orizzonte temporale infinito}
Nel caso di un \textcolor{MediumVioletRed}{\textbf{MDP a orizzonte temporale infinito}}, la funzione di valore è definita in modo implicito come soluzione di un’equazione di Bellman del tipo
\[
V(i) = \opt_{a \in \mathcal A(i)} \bigl\{ f(i,a) + \gamma \sum_j \pi(i,a,j)\, V(j) \bigr\}, \quad i \in \mathcal S,
\]
oppure, in forma equivalente,
\[
V(i) = \opt_{a \in \mathcal A(i)} \sum_{j \in \mathcal S} \pi(i,a,j)\bigl\{ h(i,a,j) + \gamma V(j) \bigr\}, \quad i \in \mathcal S.
\]
Poiché si considerano MDP finiti, lo spazio degli stati può essere identificato con un insieme finito di interi $\mathcal S = \{1,\ldots,n\}$ e la funzione di valore $V : \mathcal S \to \mathbb R$ può essere rappresentata come un vettore $\mathbf V \in \mathbb R^n$, con componenti $V(i)$, $i \in \mathcal S$. Le equazioni precedenti non forniscono una soluzione in forma chiusa, ma definiscono un problema di punto fisso.
\\ \hspace*{1.5em}
Per risolvere numericamente tali equazioni ed individuare la politica ottima, si introducono due strategie fondamentali: la \textcolor{MediumVioletRed}{\textbf{value iteration}}, basata su iterazioni computazionalmente poco costose che convergono alla funzione di valore ottima solo al limite, e la \textcolor{MediumVioletRed}{\textbf{policy iteration}}, che richiede iterazioni più onerose ma converge in tempo finito nel caso di MDP finiti, grazie al numero finito di politiche ammissibili.
\\ \hspace*{1.5em}
...
\[
[\mathcal T \tilde V](i) = \opt_{a \in \mathcal A(i)} \sum_{j \in \mathcal S} \pi(i,a,j)\bigl\{ h(i,a,j) + \gamma \tilde V(j) \bigr\}, \quad i \in \mathcal S,
\]
che associa a $\tilde{\mathbf V}$ un nuovo vettore di valori sullo spazio degli stati.
\\ \hspace*{1.5em}
Dato invece un generico vettore $\tilde{\mathbf V}$ e una politica stazionaria $\mu$, non necessariamente ottima, si definisce l’operatore $\mathcal T_\mu$ come
\[
[\mathcal T_\mu \tilde V](i) = \sum_{j \in \mathcal S} \pi(i,\mu(i),j)\bigl\{ h(i,\mu(i),j) + \gamma \tilde V(j) \bigr\}, \quad i \in \mathcal S.
\]
L’operatore $\mathcal T$ svolge un ruolo centrale nella value iteration, mentre $\mathcal T_\mu$ è alla base della policy iteration.
\\ \hspace*{1.5em}
Si osserva che il vettore di valore ottimo $\mathbf V$ è un \textbf{punto fisso} dell’operatore $\mathcal T$, ossia soddisfa
\(
\mathbf V = \mathcal T \mathbf V,
\)
mentre la funzione di valore $\mathbf V_\mu$ associata a una politica stazionaria $\mu$ è il punto fisso dell’operatore $\mathcal T_\mu$, cioè
\(
\mathbf V_\mu = \mathcal T_\mu \mathbf V_\mu.
\)
\\ \hspace*{1.5em}
Dato una politica stazionaria $\mu$, la quantità $V_\mu(i)$ rappresenta la prestazione attesa scontata ottenuta partendo dallo stato $i$ e applicando tale politica. L’esistenza di una funzione di valore e di una politica stazionaria ottima non è garantita in generale, ma è assicurata nel caso di MDP finiti con \textbf{sconto stretto} $\gamma < 1$ e contributi immediati limitati, cioè quando esiste una costante $M$ tale che $|h(i,a,j)| \le M$.
\\ \hspace*{1.5em}
L’ingrediente chiave delle dimostrazioni è il fatto che sia $\mathcal T$ sia $\mathcal T_\mu$ sono \textbf{operatori di contrazione}, e quindi ammettono un unico punto fisso. Ai fini applicativi, è sufficiente osservare che l’operatore $\mathcal T$ fornisce una caratterizzazione della politica stazionaria ottima e che, tramite una singola operazione di ottimizzazione, consente di migliorare una politica stazionaria non ottima, mentre l’operatore $\mathcal T_\mu$ permette di calcolarne la funzione di valore associata.

\subsection{Value iteration}

\paragraph{Metodo di iterazione del valore}
Secondo l’equazione \( \mathbf V = \mathcal T \mathbf V \), nel caso di \textcolor{MediumVioletRed}{\textbf{MDP finiti con sconto stretto}} \( \gamma < 1 \) il problema può essere affrontato ricercando un \textbf{punto fisso} dell’operatore di Bellman \( \mathcal T \). A tal fine, si considera un problema a orizzonte finito con funzione di valore terminale assegnata \( \mathbf V^{(0)} \) e si applica l’operatore \( \mathcal T \) ottenendo una nuova funzione di valore
\[
V^{(1)}(i) = [\mathcal T V^{(0)}](i) = \opt_{a \in \mathcal A(i)} \sum_{j \in \mathcal S} \pi(i,a,j)\bigl\{ h(i,a,j) + \gamma V^{(0)}(j) \bigr\}, \quad i \in \mathcal S,
\]
...
\[
V^{(k+1)}(i) = [\mathcal T V^{(k)}](i) = \opt_{a \in \mathcal A(i)} \sum_{j \in \mathcal S} \pi(i,a,j)\bigl\{ h(i,a,j) + \gamma V^{(k)}(j) \bigr\}, \quad i \in \mathcal S,
\]
che può essere interpretata come la ricorsione di un MDP a orizzonte finito con funzione di valore terminale \( \mathbf V^{(0)} \).
\\ \hspace*{1.5em}
L’intuizione suggerisce che, in presenza di sconto stretto, il contributo della funzione di valore terminale \( \mathbf V^{(0)} \) diventi trascurabile al crescere di \( k \), poiché è moltiplicato da un fattore \( \gamma^k \) che tende a zero. Se la successione \( \{\mathbf V^{(k)}\} \), interpretando \( k \) come contatore delle iterazioni, converge a un limite \( \mathbf V \), allora tale limite costituisce un punto fisso dell’operatore \( \mathcal T \).
\\ \hspace*{1.5em}
Più in generale, dato un operatore \( H(\cdot) : \mathbb R^n \to \mathbb R^n \), la ricerca di una soluzione dell’equazione di punto fisso \( \mathbf y = H(\mathbf y) \) può essere affrontata mediante lo schema iterativo
\(
\mathbf y^{(k+1)} = H(\mathbf y^{(k)}), \quad k = 0,1,2,\ldots,
\)
a partire da una stima iniziale \( \mathbf y^{(0)} \); tuttavia, in assenza di ulteriori proprietà, non vi è garanzia di convergenza né di esistenza o unicità del punto fisso. Nel caso in esame, il fatto che l’operatore \( \mathcal T \) sia un \textbf{operatore di contrazione} assicura invece l’esistenza e l’unicità del punto fisso, giustificando teoricamente l’algoritmo di \textbf{value iteration}.

\begin{algorithm}[H]
\caption{Value iteration (MDP finito con sconto stretto)}
\begin{algorithmic}[1]
\State Scegliere una funzione di valore iniziale $V^{(0)}$; se non vi sono indicazioni, porre $V^{(0)}(i)=0$ per ogni $i\in\mathcal S$.
\State Scegliere un parametro di tolleranza $\epsilon$.
\State Porre $k=0$ e $\texttt{stop}=\texttt{false}$.
\While{$\texttt{stop}\neq \texttt{true}$}
    \ForAll{$i\in\mathcal S$}
        \State Calcolare
        \[V^{(k+1)}(i)=\opt_{a\in\mathcal A(i)}\left\{f(i,a)+\gamma\sum_{j\in\mathcal S}\pi(i,a,j)\,V^{(k)}(j)\right\}.\]
    \EndFor
    \If{$\left\|V^{(k+1)}-V^{(k)}\right\|_{\infty}<\epsilon$}
...
\State Trovare la politica ottima stimata scegliendo un’azione arbitraria se l’insieme delle azioni ottime non è un singleton
\[
\hat\mu(i)\in\arg\opt_{a\in\mathcal A(i)}\left\{f(i,a)+\gamma\sum_{j\in\mathcal S}\pi(i,a,j)\,\hat V(j)\right\},\ \ i\in\mathcal S.
\]
\State Restituire $\hat V$ e $\hat\mu$.
\end{algorithmic}
\end{algorithm}

\paragraph{Idea dell’esempio numerico}
Si considera un semplice problema di \textbf{arresto ottimo ricorrente} modellato come una catena di Markov con quattro stati, in cui il decisore può scegliere tra le azioni \emph{wait} e \emph{reset}. Avanzando lungo la catena, la probabilità di raggiungere stati migliori diminuisce, mentre i payoff crescono con lo stato, rendendo il trade-off tra attesa e arresto fortemente dipendente dal \textbf{fattore di sconto} $\gamma$. L’esempio mostra come, al variare delle probabilità di transizione e di $\gamma$, la politica ottima passi da una strategia \emph{greedy} (arresto immediato) a politiche più pazienti, evidenziando chiaramente il ruolo del discounting nella value iteration.

\paragraph{Idea delle funzioni MATLAB}
La funzione \textcolor{MediumVioletRed}{\texttt{PW311\_FindPolicy}} implementa la \textbf{value iteration} sfruttando la struttura specifica del problema: è sufficiente descrivere le probabilità di permanenza nello stato e confrontare, a ogni iterazione, il valore dell’attesa con quello del reset. L’algoritmo restituisce la funzione di valore ottima, la politica binaria associata e il numero di iterazioni necessarie alla convergenza. La funzione \textcolor{MediumVioletRed}{\texttt{PW311\_Simulate}} utilizza invece una \textbf{simulazione Monte Carlo} per stimare le prestazioni scontate di una politica fissata, consentendo di confrontare i valori stimati con quelli teorici e di valutarne la robustezza rispetto a possibili errori di modellazione.

\subsection{Policy iteration}
\paragraph{Idea e formulazione matematica}
Con la \textcolor{MediumVioletRed}{\textbf{policy iteration}} può accadere che, a differenza della value iteration, non ci si accontenti di individuare rapidamente una politica ottima basata su valori approssimati degli stati, ma si miri a valutare in modo \emph{esatto} il valore di una politica fissata. Si consideri l’operatore $T_\mu$ associato a una politica stazionaria $\mu$, definito come
\[
[T_\mu \tilde V](i)
=
f\bigl(i,\mu(i)\bigr)
+
\gamma \sum_{j\in\mathcal S} \pi\bigl(i,\mu(i),j\bigr)\tilde V(j),
\qquad i\in\mathcal S.
\]
\hspace*{1.5em}A differenza dell’operatore di Bellman $T$, l’operatore $T_\mu$ non coinvolge alcuna ottimizzazione ed è interamente determinato dalla politica $\mu$. La funzione di valore $V_\mu$ associata alla politica $\mu$ è un punto fisso di $T_\mu$ e soddisfa, per ogni $i\in\mathcal S$, il sistema di equazioni lineari
\[
V_\mu(i)
=
f\bigl(i,\mu(i)\bigr)
+
\gamma \sum_{j\in\mathcal S} \pi\bigl(i,\mu(i),j\bigr) V_\mu(j).
\]
Raccogliendo le probabilità di transizione indotte da $\mu$ nella matrice
\[
\Pi_\mu :=
\begin{bmatrix}
\pi(1,\mu(1),1) & \pi(1,\mu(1),2) & \cdots & \pi(1,\mu(1),n) \\
\pi(2,\mu(2),1) & \pi(2,\mu(2),2) & \cdots & \pi(2,\mu(2),n) \\
\vdots & \vdots & \ddots & \vdots \\
\pi(n,\mu(n),1) & \pi(n,\mu(n),2) & \cdots & \pi(n,\mu(n),n)
\end{bmatrix},
\]
...
\end{bmatrix},
\]
il problema di valutazione della politica può essere scritto in forma compatta come
\(
V_\mu = f_\mu + \gamma \Pi_\mu V_\mu,
\)
ossia
\(
(\mathbf I - \gamma \Pi_\mu)V_\mu = f_\mu,
\)
dove $\mathbf I\in\mathbb R^{n\times n}$ è la matrice identità. Formalmente, la soluzione è data da
\(
V_\mu = (\mathbf I - \gamma \Pi_\mu)^{-1} f_\mu.
\)
\\ \hspace*{1.5em}Sebbene l’inversione matriciale fornisca una soluzione chiusa, per problemi di grandi dimensioni essa risulta computazionalmente onerosa e distrugge la sparsità della matrice, rendendo preferibili metodi iterativi. Una volta calcolato $V_\mu$, è possibile migliorare la politica definendo
\[
\tilde\mu(i)\in \arg\opt_{a\in\mathcal A(i)}
\left\{
f(i,a)
+
\gamma \sum_{j\in\mathcal S}\pi(i,a,j)V_\mu(j)
\right\},
\qquad i\in\mathcal S.
\]
\hspace*{1.5em}Questa operazione, detta \textbf{policy improvement}, garantisce che la nuova politica non sia peggiore della precedente, cioè
\(
V_\mu(i) \le V_{\tilde\mu}(i), \; i\in\mathcal S,
\)
nel caso di un problema di massimizzazione, con disuguaglianza stretta in almeno uno stato se $\mu$ non è ottima. Poiché in un MDP finito il numero di politiche stazionarie deterministiche è finito, una sequenza di valutazioni e miglioramenti conduce necessariamente a una politica ottima, che costituisce il fondamento dell’algoritmo di policy iteration.

\begin{algorithm}[H]
\caption{Policy iteration}
\begin{algorithmic}[1]
\State Definire una politica stazionaria iniziale arbitraria $\mu^{(0)}$.
\State Porre $k=0$ e \texttt{stop = false}.
\While{\texttt{stop} $\neq$ \texttt{true}}
    \State Valutare la politica $\mu^{(k)}$ risolvendo
    \[
    \bigl(\mathbf I-\gamma \Pi_{\mu^{(k)}}\bigr)\,V_{\mu^{(k)}}=f_{\mu^{(k)}}.
    \]
    \State Trovare una nuova politica stazionaria $\mu^{(k+1)}$ tramite \emph{policy improvement}
    \[
    \mu^{(k+1)}(i)\in \arg\opt_{a\in\mathcal A(i)}
    \left\{ f(i,a)+\gamma\sum_{j\in\mathcal S}\pi(i,a,j)V_{\mu^{(k)}}(j)\right\},
    \qquad i\in\mathcal S.
    \]
    \If{$\mu^{(k+1)}=\mu^{(k)}$}
        \State \texttt{stop = true}.
    \Else
        \State $k \leftarrow k+1$.
    \EndIf
\EndWhile
\State Restituire $V_{\mu^{(k)}}$ e $\mu^{(k)}$.
\end{algorithmic}
\end{algorithm}

\paragraph{Idea dell’implementazione MATLAB}
La funzione \textcolor{MediumVioletRed}{\texttt{FindPolicyPI}} realizza una versione generale e diretta della \emph{policy iteration}, alternando esplicitamente le fasi di \emph{policy evaluation} e \emph{policy improvement}. A partire da una politica iniziale \textcolor{MediumVioletRed}{\texttt{initialPolicy}}, a ogni iterazione viene costruito il sistema lineare $(\mathbf I-\gamma\Pi_\mu)V_\mu=f_\mu$ tramite la funzione annidata \texttt{makeArrays} e risolto con eliminazione di Gauss per ottenere la funzione di valore della politica corrente. Successivamente, la politica viene migliorata stato per stato scegliendo l’azione che minimizza o massimizza il valore atteso, tramite \texttt{findMin} o \texttt{findMax}, fino a quando la politica non cambia più. L’output finale consiste nel vettore \texttt{stateValues}, che approssima $V_\mu$, e nel vettore \texttt{policy}, che rappresenta la politica stazionaria ottima.

\paragraph{Esempio numerico}
Dall’esempio numerico si evince che la \textbf{policy iteration} converge in un numero molto ridotto di passi rispetto alla value iteration. Partendo dalla politica iniziale, che prescrive di attendere ogniqualvolta possibile, la valutazione della politica e il successivo passo di \emph{policy improvement} portano immediatamente alla nuova politica, che risulta già ottima. Questo comportamento conferma che, una volta calcolata esattamente la funzione di valore $V_\mu$, il miglioramento della politica può produrre variazioni significative in un singolo passo, evidenziando l’efficienza concettuale della policy iteration nel caso di MDP finiti con struttura semplice.

\paragraph{Value iteration vs.\ policy iteration}
A prima vista, la \textbf{value iteration} e la \textbf{policy iteration} appaiono molto diverse. La value iteration si basa su un numero potenzialmente elevato di iterazioni computazionalmente poco costose, mentre la policy iteration si fonda su un numero (auspicabilmente ridotto) di iterazioni che possono essere computazionalmente onerose. Inoltre, la convergenza della value iteration è garantita solo al limite, mentre per la policy iteration si ottiene una convergenza in tempo finito. È tuttavia possibile colmare il divario tra i due approcci. Si consideri la valutazione di una politica stazionaria $\mu$ mediante iterazione del punto fisso dell’operatore $T_\mu$, secondo
\[
V^{(k+1)}_\mu(i)
=
f\bigl(i,\mu(i)\bigr)
+
\gamma \sum_{j\in\mathcal S}\pi\bigl(i,\mu(i),j\bigr)V^{(k)}_\mu(j),
\qquad i\in\mathcal S.
\]
\hspace*{1.5em}In pratica, questa procedura è necessaria quando l’\emph{eliminazione di Gauss} per la risoluzione di grandi sistemi lineari non è applicabile. Idealmente, prima del passo di miglioramento si dovrebbe attendere la convergenza completa dello schema precedente; tuttavia, se si interrompe prematuramente l’iterazione assumendo in modo ottimistico di aver valutato correttamente la politica corrente, si ottiene una stima $\hat V_\mu(i)=V^{(k+1)}_\mu(i)$ e si applica il passo di miglioramento
\[
\tilde\mu(i)\in \arg\opt_{a\in\mathcal A(i)}
\left\{
f(i,a)+\gamma \sum_{j\in\mathcal S}\pi(i,a,j)\hat V_\mu(j)
\right\},
\qquad i\in\mathcal S.
\]
\hspace*{1.5em}Questo approccio prende il nome di \textcolor{MediumVioletRed}{\textbf{optimistic policy iteration}} ed è alla base della cosiddetta \textbf{generalized policy iteration}. A seconda del grado di ottimismo nella valutazione della politica, l’equazione precedente può essere iterata per un numero elevato o ridotto di passi. Nella value iteration, la politica viene invece aggiornata implicitamente a ogni iterazione tramite
\[
V^{(k+1)}(i)
=
\opt_{a\in\mathcal A(i)}
\sum_{j\in\mathcal S}\pi(i,a,j)\bigl\{h(i,a,j)+\gamma V^{(k)}(j)\bigr\},
\qquad i\in\mathcal S,
\]
applicando ripetutamente l’operatore $T$ senza mai valutare esplicitamente $T_\mu$. Al contrario, la policy iteration applica molte iterazioni di $T_\mu$ prima di passare a una nuova politica. Pertanto, value iteration e policy iteration possono essere viste come gli estremi di un continuo di approcci correlati.
\\ \hspace*{1.5em}Questa distinzione diventa particolarmente rilevante nel contesto del \textcolor{MediumVioletRed}{\textbf{reinforcement learning}}. Quando le probabilità di transizione (e i contributi immediati) non sono note, si ricorre a una programmazione dinamica \emph{model-free}, in cui la funzione di valore è sostituita dai \textbf{Q-factors} $Q(s,a)$ dipendenti sia dallo stato sia dall’azione. In questo contesto, il \textbf{Q-learning} rappresenta il corrispettivo della value iteration ed è uno schema di apprendimento \emph{off-policy}, mentre i metodi ispirati alla policy iteration sono tipicamente \emph{on-policy}, come nel caso dell’algoritmo \textbf{SARSA}. In reinforcement learning non è in genere possibile apprendere esattamente la funzione di valore di una politica, soprattutto se ciò richiede simulazioni Monte Carlo costose o esperimenti online, rendendo inevitabile l’esecuzione anticipata di passi di miglioramento e dando origine a una vasta famiglia di strategie di apprendimento.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% PDF 11 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\vspace{-1cm}
\textcolor{DarkViolet}{\section{CAP 11 -- Programmazione dinamica approssimativa e RL per stati discreti}}
\paragraph{Recap}
La \textcolor{DarkViolet}{\textbf{programmazione dinamica approssimativa (ADP)}} raccoglie metodi numerici che rinunciano alla garanzia di ottimalità per attenuare le \textcolor{DarkViolet}{\textbf{maledizioni della DP}} e rendere affrontabili MDP finiti a \textbf{orizzonte infinito} anche in assenza di un modello esplicito. Nel contesto \textbf{model-free}, le versioni approssimate di value iteration e policy iteration rientrano nel \textcolor{DarkViolet}{\textbf{reinforcement learning (RL)}}, in cui un agente apprende interagendo con il sistema osservando stato, contributo immediato e stato successivo. Un punto chiave è che la sola funzione di valore sugli stati non basta per valutare le azioni senza informazioni di transizione; per questo si introducono i \textcolor{DarkViolet}{\textbf{valori stato--azione}} tramite \textbf{Q-factor}, assumendo (quando possibile) una rappresentazione tabellare oppure ricorrendo a rappresentazioni compatte.
\\ \hspace*{1.5em}
L’apprendimento tramite campionamento porta a due difficoltà strutturali, il compromesso tra \textcolor{DarkViolet}{\textbf{exploration}} ed \textcolor{DarkViolet}{\textbf{exploitation}} e la \textbf{non stazionarietà} del bersaglio, dovuta al fatto che la politica evolve durante l’apprendimento. Per gestire il compromesso si impiegano politiche casuali, come $\varepsilon$-greedy (statico o dinamico) e \textcolor{DarkViolet}{\textbf{Boltzmann exploration}} basata su soft-max, mentre la non stazionarietà motiva aggiornamenti incrementali con \textcolor{DarkViolet}{\textbf{smoothing esponenziale}} e learning rate costante o decrescente. L’esempio dei \textbf{multi-armed bandits} mostra come campioni piccoli e varianza elevata rendano facile selezionare azioni subottimali, giustificando la necessità di exploration controllata.
\\ \hspace*{1.5em}
Nel quadro dei metodi a \textbf{temporal differences} si ottiene \textcolor{DarkViolet}{\textbf{SARSA}}, schema \textbf{on-policy} che apprende i Q-factor di una politica stazionaria tramite correzioni smussate dal learning rate e procedure di \textcolor{DarkViolet}{\textbf{bootstrapping}}; l’aggiornamento della politica prima di una valutazione esatta conduce alla \textcolor{DarkViolet}{\textbf{generalized (optimistic) policy iteration}}, che può accelerare l’apprendimento ma indebolire la convergenza. In alternativa, il \textcolor{DarkViolet}{\textbf{Q-learning}} apprende direttamente i Q-factor \textbf{ottimi} con logica \textbf{off-policy}, utilizzando un target che combina contributo immediato e valore ottimo stimato dello stato successivo, e aggiornamenti in stile smoothing esponenziale o temporal difference. La differenza concettuale con SARSA emerge nel fatto che SARSA aggiorna rispetto all’azione scelta dalla politica corrente, mentre Q-learning usa l’azione ottimale (stimata) nello stato successivo.
\\ \hspace*{1.5em}
Le implementazioni MATLAB evidenziano la separazione tra algoritmo e simulatore tramite un oggetto di sistema (\texttt{systemObj}/\texttt{systemClass}) che fornisce contributo e transizione, il conteggio delle visite stato--azione e la possibilità di integrare una scelta \textcolor{DarkViolet}{\textbf{$\varepsilon$-greedy}}. L’esempio numerico mostra che l’esito pratico del Q-learning dipende in modo critico da inizializzazione, learning rate e exploration: senza exploration alcune azioni possono non essere mai provate e i relativi Q-factor non vengono aggiornati, mentre una strategia $\varepsilon$-greedy può recuperare la politica ottima, a costo di maggiore effort computazionale.


\paragraph{ADP e programmazione dinamica model-free}
La \textcolor{DarkViolet}{\textbf{approximate dynamic programming (ADP)}} è un termine ombrello che identifica un’ampia classe di metodi numerici di programmazione dinamica che, a differenza dei metodi standard, \textbf{non garantiscono il raggiungimento di una politica ottima}. In cambio della loro natura approssimata, tali metodi mirano ad attenuare le ben note \textcolor{DarkViolet}{\textbf{maledizioni della DP}}, rendendo trattabili problemi che sarebbero altrimenti computazionalmente proibitivi. Si considerano qui metodi approssimati per \textbf{MDP finiti} nel contesto a \textbf{orizzonte infinito}.
\\ \hspace*{1.5em}Le versioni \textbf{model-free} sia della value iteration sia della policy iteration rientrano collettivamente nel quadro del \textcolor{DarkViolet}{\textbf{reinforcement learning (RL)}}. In tale impostazione, un \textbf{agente} interagisce con un sistema al fine di apprendere una politica di controllo adeguata: non si dispone di un modello del sistema, ma si osservano lo stato corrente e l’esito dell’azione scelta, costituito dal contributo immediato e dallo stato successivo. Una buona politica deve bilanciare obiettivi di breve e di lungo periodo, valutati rispettivamente tramite il contributo immediato e il valore dello stato futuro.
\\ \hspace*{1.5em}Un punto cruciale del RL model-free è che, anche disponendo di una conoscenza perfetta della funzione di valore degli stati, non è possibile valutare correttamente un’azione in assenza di informazioni sulle transizioni. Per ovviare a questo limite si introducono i \textcolor{DarkViolet}{\textbf{valori stato--azione}}, sotto forma di \textbf{Q-factor}, che consentono di apprendere una politica decisionale tramite strategie di campionamento. Si assume inizialmente che lo spazio stato--azione non sia eccessivamente grande e che una rappresentazione tabellare dei Q-factor sia praticabile; nella maggior parte dei casi, tuttavia, ciò non è vero e diventa necessario ricorrere a rappresentazioni compatte.

\paragraph{Campionamento e stima in contesti non stazionari}
Nella teoria della probabilità si è spesso interessati al \textcolor{DarkViolet}{\textbf{valore atteso}} di una funzione $h(\cdot)$ di una variabile aleatoria o di un vettore di variabili aleatorie, definito come
$$\theta = \mathbb{E}[h(X)] = \int_{\mathcal{X}} h(x) f_X(x)\,dx,$$
dove $\mathcal{X}$ rappresenta il supporto della distribuzione.  
\\ \hspace*{1.5em}
Poiché la valutazione di integrali ad alta dimensionalità risulta in generale complessa, si ricorre a tecniche di \textcolor{DarkViolet}{\textbf{campionamento Monte Carlo}}, che permettono di approssimare il valore atteso mediante una media campionaria, ossia
$$\hat{\theta} = \frac{1}{m}\sum_{k=1}^m h\!\left(X^{(k)}\right).$$
Tale stima risulta \textbf{non distorta}, in quanto il suo valore atteso coincide con il valore vero di $\theta$.  
\\ \hspace*{1.5em}
Nel contesto dei \textbf{processi decisionali di Markov}, tuttavia, l’obiettivo non è la stima di una quantità statica, bensì dei \textcolor{DarkViolet}{\textbf{valori di stato}} $V(i)$ o dei \textcolor{DarkViolet}{\textbf{Q-factor}} $Q(i,a)$. In questa impostazione, l’apprendimento statistico deve confrontarsi con due principali difficoltà: da un lato la necessità di bilanciare \textbf{exploration} e \textbf{exploitation}, dall’altro la complessità legata alla stima di un \textbf{bersaglio non stazionario}, che evolve nel tempo man mano che la politica viene aggiornata.

\paragraph{Il compromesso tra exploration ed exploitation}
Si supponga di voler apprendere i \textcolor{DarkViolet}{\textbf{Q-factor}} $Q(i,a)$ tramite campionamento e di trovarsi attualmente nello stato $i$. Sorge allora il problema di stabilire quale azione selezionare. Una possibilità consiste nel fare affidamento sulle stime correnti dei Q-factor e scegliere l’azione che appare più promettente. Questa strategia risulta sensata solo quando il valore associato a ciascuna azione nello stato considerato è stato stimato con sufficiente accuratezza.  
\\ \hspace*{1.5em}
All’inizio del processo di apprendimento, tuttavia, tale condizione è difficilmente soddisfatta. Può infatti esistere un’azione che sembra poco conveniente semplicemente perché il suo contributo immediato è stato valutato in modo impreciso; inoltre, tale azione potrebbe condurre a uno stato favorevole, del quale non si ha ancora conoscenza. Queste considerazioni conducono al problema fondamentale del bilanciamento tra \textcolor{DarkViolet}{\textbf{exploration}} delle alternative poco conosciute ed \textcolor{DarkViolet}{\textbf{exploitation}} delle informazioni attualmente disponibili.  
\\ \hspace*{1.5em}
...
\\ \hspace*{1.5em}
Si supponga invece di poter ottenere solo stime rumorose $\hat{\nu}(a)$ tramite campionamento. Seguendo una strategia di \textbf{pura exploitation}, si sceglierebbe l’azione che massimizza $\hat{\nu}(\cdot)$; tuttavia, una tale scelta può rivelarsi inadeguata, poiché trascura l’incertezza associata alle stime e il potenziale informativo derivante dall’exploration di azioni meno conosciute.
\\ \hspace*{1.5em}
All’estremo opposto rispetto alla pura exploitation si colloca la \textbf{pura exploration}, che consiste nella selezione casuale delle azioni secondo probabilità uniformi. È evidente come nessuna di queste due strategie estreme risulti soddisfacente, rendendo necessario individuare un compromesso ragionevole tra exploration ed exploitation.  
\\ \hspace*{1.5em}
Una prima possibilità è rappresentata dall’approccio \textcolor{DarkViolet}{\textbf{$\varepsilon$-greedy statico}}, in cui si fissa una probabilità $\varepsilon$ e si seleziona l’azione più promettente con probabilità $1-\varepsilon$, mentre con probabilità $\varepsilon$ si sceglie un’azione casuale. In questo modo si introduce una componente esplorativa controllata, pur privilegiando le azioni che appaiono migliori sulla base delle stime correnti.  
\\ \hspace*{1.5em}
Un raffinamento naturale consiste nell’adottare un approccio \textcolor{DarkViolet}{\textbf{$\varepsilon$-greedy dinamico}}, in cui il parametro $\varepsilon$ viene fatto variare nel tempo, ad esempio ponendo $\varepsilon^{(k)} = \frac{c}{d+k}$ oppure $\varepsilon^{(k)} = d + \frac{c}{k}$ all’iterazione $k$. In tal modo l’exploration risulta più marcata nelle fasi iniziali dell’apprendimento e viene progressivamente ridotta man mano che le stime diventano più affidabili.  
\\ \hspace*{1.5em}
Un’alternativa più sofisticata è fornita dalla \textcolor{DarkViolet}{\textbf{Boltzmann exploration}}, che definisce una politica stocastica mediante una funzione soft-max, assegnando a ciascuna azione $a \in A$ una probabilità
$$\varepsilon(a) = \dfrac{\exp\!\big(\rho\,\hat{\nu}(a)\big)}{\sum_{a' \in A} \exp\!\big(\rho\,\hat{\nu}(a')\big)}.$$
Il parametro $\rho$ regola il compromesso tra exploration ed exploitation: per $\rho = 0$ si ottiene una scelta puramente casuale, mentre nel limite $\rho \to +\infty$ la politica converge verso una pura exploitation.
\\ \hspace*{1.5em}
Si osserva quindi che, per gestire il compromesso tra exploration ed exploitation, si ricorre in modo sistematico a \textbf{politiche casuali}, nelle quali la selezione delle azioni incorpora deliberatamente una componente stocastica.


\paragraph{Osservazioni sull’esempio dei multi-armed bandits}
Dall’esempio numerico si evince come, in un problema di \textcolor{DarkViolet}{\textbf{multi-armed bandit}}, l’azione ottima in termini di valore atteso possa risultare difficile da individuare quando il numero di osservazioni è limitato. In particolare, sebbene il terzo braccio presenti il valore atteso più elevato, esso è anche caratterizzato da una variabilità significativamente maggiore rispetto agli altri.  
\\ \hspace*{1.5em}
A causa dell’elevata varianza, le medie campionarie calcolate su pochi campioni possono discostarsi sensibilmente dal valore atteso, producendo stime che rendono temporaneamente più attrattivi bracci subottimali. Ne consegue che, con una dimensione campionaria ridotta, è altamente probabile selezionare un’azione diversa da quella realmente ottima.  
\\ \hspace*{1.5em}
L’esempio mette quindi in evidenza il ruolo cruciale dell’incertezza statistica nella fase di apprendimento e mostra come una strategia basata esclusivamente sulle stime correnti possa condurre a decisioni errate. Ciò giustifica la necessità di meccanismi che favoriscano un’adeguata exploration, soprattutto nelle fasi iniziali, al fine di ridurre il rischio di convergere prematuramente verso scelte subottimali.

\paragraph{Non stazionarietà e smoothing esponenziale}
Anche qualora il sistema sottostante sia stazionario, la politica utilizzata per prendere decisioni non lo è, poiché viene aggiornata progressivamente durante il processo di apprendimento. A titolo di confronto, si consideri la media campionaria utilizzata per stimare il valore atteso $\theta = \mathbb{E}[X]$ di una variabile aleatoria scalare $X$, nella quale tutte le osservazioni raccolte contribuiscono con lo stesso peso.  
\\ \hspace*{1.5em}
...
Questo schema prende il nome di \textcolor{DarkViolet}{\textbf{smoothing esponenziale}}.  
\\ \hspace*{1.5em}
...
dove $\hat{\theta}^{(0)}$ è una stima iniziale. A differenza della media campionaria standard, il peso delle osservazioni passate decresce esponenzialmente. Il coefficiente $\alpha$ è noto come \textcolor{DarkViolet}{\textbf{learning rate}}, o coefficiente di smoothing, o fattore di dimenticanza.  
\\ \hspace*{1.5em}
Valori elevati di $\alpha$ producono aggiornamenti più reattivi ma anche più instabili, mentre valori piccoli introducono maggiore inerzia e filtraggio del rumore. Se la non stazionarietà è dovuta esclusivamente al processo di apprendimento e non a variazioni nella dinamica del sistema, è possibile considerare una sequenza decrescente di learning rate (step sizes) $\alpha^{(k)}$, da applicare al passo $k$, tale che
$$\sum_{k=1}^{\infty} \alpha^{(k)} = \infty \quad \text{ e } \quad \sum_{k=1}^{\infty}\left[\alpha^{(k)}\right]^2 < \infty.$$

\paragraph{Learning tramite temporal differences e algoritmo SARSA}
Si è introdotto l’operatore $\mathcal{T}_\mu$, definito come $$[\mathcal{T}_\mu \tilde V](i)=\sum_{j\in S}\pi(i,\mu(i),j)\big(h(i,\mu(i),j)+\gamma \tilde V(j)\big),$$ dove $\mu$ è una politica stazionaria e $\tilde V$ è una funzione di valore. Il valore $V_\mu$ associato alla politica $\mu$ può essere ottenuto come punto fisso dell’operatore, risolvendo l’equazione $\mathcal{T}_\mu V_\mu = V_\mu$.  
\\ \hspace*{1.5em}
La stessa equazione di punto fisso può essere riscritta in termini di \textcolor{DarkViolet}{\textbf{Q-factor}} associati alla politica $\mu$, ottenendo $$Q_\mu(i,\mu(i))=\mathbb{E}\!\left[h(i,\mu(i),j)+\gamma Q_\mu(j,\mu(j))\right],$$ dove l’aspettativa è presa rispetto allo stato successivo $j$. Astrattamente, il problema può essere ricondotto a un’equazione di punto fisso del tipo $y=Hy$, la cui soluzione tramite iterazione diretta $y^{(k)}=Hy^{(k-1)}$ non garantisce in generale la convergenza.  
\\ \hspace*{1.5em}
Anche nel caso in cui l’operatore $H$ sia una contrazione, non è possibile valutarlo esattamente poiché la distribuzione di probabilità necessaria a calcolare l’aspettativa non è nota. Si riscrive allora l’equazione come $y=y+\alpha(Hy-y)$, da cui deriva lo schema iterativo $y^{(k)}=y^{(k-1)}+\alpha\big(Hy^{(k-1)}-y^{(k-1)}\big)$, che consente di applicare uno smoothing esponenziale sostituendo le aspettative con osservazioni casuali.  
\\ \hspace*{1.5em}
Applicando tale schema al contesto dei processi decisionali di Markov, si consideri di trovarsi allo stato $s^{(k)}=i$ e di applicare l’azione $a^{(k)}=\mu(i)$, osservando lo stato successivo $s^{(k+1)}=j$ e il contributo immediato $h(i,\mu(i),j)$. L’adattamento dello schema porta all’aggiornamento definito dalla \textcolor{DarkViolet}{\textbf{temporal difference}} $$\Delta^{(k)} = h(i,\mu(i),j)+\gamma \hat Q_\mu^{(k-1)}(j,\mu(j))-\hat Q_\mu^{(k-1)}(i,\mu(i))$$ e dalla relazione $$\hat Q_\mu^{(k)}(i,\mu(i))=\hat Q_\mu^{(k-1)}(i,\mu(i))+\alpha\,\Delta^{(k)}.$$ Il valore atteso della temporal difference sarebbe nullo se fossero noti i veri Q-factor; in pratica, si utilizza una singola osservazione casuale e una correzione non nulla indica la necessità di aggiornare la stima corrente, con un fattore di smoothing pari al learning rate $\alpha$.  
\\ \hspace*{1.5em}
Poiché il metodo utilizza stime per costruire nuove stime, esso rientra nella classe dei metodi di \textcolor{DarkViolet}{\textbf{bootstrapping}}. L’algoritmo così ottenuto prende il nome di \textcolor{DarkViolet}{\textbf{SARSA}}, acronimo della sequenza (State, Action, Reward, State, Action), ed è un metodo \textcolor{DarkViolet}{\textbf{on-policy}}, poiché il comportamento dell’agente segue la politica $\mu$ di cui si vuole apprendere il valore, come avviene nella policy iteration.  
\\ \hspace*{1.5em}
Per rendere operativo l’algoritmo sono necessari un adeguato livello di exploration, ad esempio tramite una strategia $\varepsilon$-greedy, e l’arresto anticipato dell’apprendimento per eseguire un passo di miglioramento della politica. Questo schema prende il nome di \textcolor{DarkViolet}{\textbf{generalized (o optimistic) policy iteration}}, in quanto si assume in modo ottimistico di disporre di una valutazione sufficientemente accurata della politica corrente. Tale scelta può accelerare l’apprendimento, ma può avere effetti negativi sulla convergenza. Una volta ottenuta una stima $\hat Q_\mu(i,a)$ dei Q-factor, la politica viene aggiornata in modo greedy ponendo $$\tilde\mu(i)\in\arg\opt_{a\in A(i)}\hat Q_\mu(i,a);$$ a differenza dell’iterazione esatta, non vi è tuttavia garanzia che la nuova politica risulti effettivamente migliorata.

\paragraph{Q-learning per MDP finiti}
Il \textcolor{DarkViolet}{\textbf{Q-learning}} è un approccio di reinforcement learning \textbf{model-free} alternativo per il trattamento di MDP finiti. Si richiamano innanzitutto le equazioni fondamentali della programmazione dinamica in termini di \textbf{Q-factor ottimi}, date da
$$
Q(i,a)=\sum_{j\in S}\pi(i,a,j)\Big(h(i,a,j)+\gamma \opt_{a'\in A(j)} Q(j,a')\Big), 
\qquad i\in S,\; a\in A(i),
$$
e dalla relazione
$$
...
$$
A differenza del caso SARSA, in cui si considerano Q-factor associati a una politica stazionaria fissata $\mu$, in questo contesto si lavora direttamente con i fattori \textbf{ottimi}.  
\\ \hspace*{1.5em}
...
$$
\hspace*{1.5em}In questo senso, il Q-learning applica la stessa logica \textbf{off-policy} utilizzata nella value iteration. A differenza della policy iteration, non si valuta una politica fissata $\mu$: la politica è implicita nei Q-factor e, poiché questi vengono aggiornati continuamente, la politica cambia nel tempo; in altre parole, si utilizza una politica per apprenderne un’altra.  
\\ \hspace*{1.5em}
...
$$
\hat q = h\big(i,a^{(k)},s^{(k+1)}\big)
+ \gamma \opt_{a'\in A(s^{(k+1)})} \hat Q^{(k-1)}\big(s^{(k+1)},a'\big),
$$
...
$$
\hat V^{(k-1)}\big(s^{(k+1)}\big)
= \opt_{a'\in A(s^{(k+1)})} \hat Q^{(k-1)}\big(s^{(k+1)},a'\big).
$$
\\ \hspace*{1.5em}
Il Q-factor relativo alla coppia stato–azione $\big(s^{(k)},a^{(k)}\big)$ viene quindi aggiornato secondo la logica dello \textcolor{DarkViolet}{\textbf{smoothing esponenziale}},
$$
\hat Q^{(k)}\big(s^{(k)},a^{(k)}\big)
= \alpha \hat q + (1-\alpha)\hat Q^{(k-1)}\big(s^{(k)},a^{(k)}\big),
$$
assumendo un coefficiente di smoothing costante $\alpha$, sebbene sia possibile considerare anche coefficienti decrescenti. La stessa idea può essere espressa in termini di \textbf{temporal difference} definendo
$$
\Delta^{(k)} =
\Big[h\big(s^{(k)},a^{(k)},j\big)
+ \gamma \opt_{a'\in A(j)} \hat Q^{(k-1)}(j,a')\Big]
...
\hat Q^{(k)}(s,a)=
\begin{cases}
\hat Q^{(k-1)}(s,a)+\alpha\,\Delta^{(k)}, & \text{se } s=s^{(k)},\ a=a^{(k)},\\
\hat Q^{(k-1)}(s,a), & \text{altrimenti},
\end{cases}
$$
dove $j=s^{(k+1)}$ è lo stato successivo osservato.  
\\ \hspace*{1.5em}
La differenza tra SARSA e Q-learning emerge confrontando le correzioni introdotte nei due casi: entrambe aggiornano le stime dei valori stato–azione, ma SARSA è un metodo \textbf{on-policy}, riferito a una politica stazionaria $\mu$ fissata, mentre il Q-learning è \textbf{off-policy}, poiché la politica continua a cambiare durante il processo di apprendimento.

\paragraph{Implementazione MATLAB del Q-learning}
Una possibile implementazione del Q-learning in ambiente MATLAB è riportata nel codice mostrato, che traduce operativamente lo schema teorico discusso in precedenza. La funzione \textcolor{DarkViolet}{\texttt{QLearning}} restituisce i Q-factor appresi, la politica stazionaria associata e una matrice che conta le visite delle coppie stato--azione, permettendo di monitorare il processo di apprendimento.  
\\ \hspace*{1.5em}
L’algoritmo interagisce con il sistema dinamico tramite un oggetto \texttt{systemObj}, che fornisce il contributo immediato e lo stato successivo in risposta a un’azione selezionata. A ogni iterazione l’azione viene scelta sulla base delle stime correnti dei Q-factor, con la possibilità di introdurre exploration mediante una strategia \textcolor{DarkViolet}{\textbf{$\varepsilon$-greedy}}.  
\\ \hspace*{1.5em}
Dopo l’osservazione della transizione, il Q-factor corrispondente viene aggiornato tramite uno schema di \textbf{smoothing esponenziale}, combinando la nuova informazione con la stima precedente mediante il learning rate $\alpha$. Al termine dell’apprendimento, la politica viene ricavata in modo greedy dai Q-factor, fornendo una realizzazione compatta e coerente dell’approccio \textbf{off-policy} del Q-learning per MDP finiti.

\paragraph{Definizione MATLAB di una classe di sistema generica}
Poiché il Q-learning è un approccio \textbf{model-free}, è necessario mantenere separato l’algoritmo di apprendimento dalla simulazione del sistema dinamico. Tale separazione è ottenuta tramite la classe MATLAB \textcolor{DarkViolet}{\texttt{systemClass}}, che funge da interfaccia tra il Q-learner e il sistema da controllare, incapsulando stati, azioni ammissibili, probabilità di transizione e contributi immediati.  
\\ \hspace*{1.5em}
Il metodo centrale della classe è \texttt{getNext}, che, dato lo stato corrente e un’azione selezionata, restituisce il contributo immediato e lo stato successivo campionato secondo le probabilità di transizione, aggiornando lo stato interno dell’oggetto. In questo schema il contributo è associato alla coppia stato–azione, assumendo che sia deterministico oppure noto in valore atteso.  
\\ \hspace*{1.5em}
...
T = 60; % Orizzonte temporale (giorni fino alla partenza)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\textcolor{DarkViolet}{\section{RECAP -- MATLAB}}

\paragraph{Obiettivo}
Schemi MATLAB minimi per riconoscere la tipologia di esercizio e scrivere lo scheletro corretto tra programmazione dinamica, enumerazione discreta e simulazione.

\paragraph{1. Sintassi MATLAB da memorizzare}
Sintassi di base da ricordare per scrivere i codici.
\begin{lstlisting}[style=matlabStyle]
% script
clear;
clc;

% vettori e matrici + preallocazione
v = [1; 2; 3];          % colonna
w = [1 2 3];            % riga
M = zeros(n,m);         % preallocazione

% indicizzazione (stati spesso da 0)
V(state+1,t+1);
policy(state+1,t);

% for (backward tipico in DP)
for t = T:-1:1
    % codice
end

% while (iterazioni fino a tolleranza)
diff = Inf;
tol  = 1e-6;
while diff > tol
    % aggiorna diff
...
% if / elseif / else (condizione esplicita)
x = 3;
if x > 0
    y = 1;
elseif x == 0
...
\paragraph{2. Programmazione dinamica backward} Esempio di programmazione dinamica.
\begin{lstlisting}[style=matlabStyle]
for t = T:-1:1
    for s = 0:maxState
        
        value = zeros(numActions,1);
        
        for a = 1:numActions
            value(a) = reward + beta*V(nextState+1,t+1);
        end
        
        [V(s+1,t), policy(s+1,t)] = max(value);
        
    end
end
\end{lstlisting}

\paragraph{3. Enumerazione discreta} Esempio di enumerazione discreta per trovare la soluzione ottima.
\begin{lstlisting}[style=matlabStyle]
for q = 0:maxQ
    profit(q+1) = computeProfit(q);
end
[maxProfit, qStar] = max(profit);
\end{lstlisting}

\paragraph{4. Valore atteso su scenari} Calcolo del valore atteso dato un vettore di probabilità e payoff.
\begin{lstlisting}[style=matlabStyle]
EV = sum(prob .* payoff);
\end{lstlisting}

\paragraph{5. Simulazione a eventi discreti} Struttura generale di un simulatore a eventi discreti.

\begin{lstlisting}[style=matlabStyle]
% struttura DES
clock = 0;
eventTimes = [nextArrival; nextCompletion];
...
eventTimes = [exprnd(1/arrivalRate); inf];
...
while clock <= horizon
    ...
    switch idx
        ...
        case 1
            ...
            if serverState == idle
                serverState = busy;
                serviceTime = unifrnd(a,b);
                eventTimes(2) = clock + serviceTime;
...
\paragraph{Distribuzioni da ricordare} Generazione di variabili casuali con distribuzioni comuni.
\begin{lstlisting}[style=matlabStyle]
exprnd(1/lambda)
unifrnd(a,b)
poissrnd(lambda*T)
...
\paragraph{6. Simulazione time--driven} Struttura generale di un simulatore time--driven.
...
\begin{lstlisting}[style=matlabStyle]
% scheletro time-driven (inventario con shelf life)
horizon = 300;
S = 100;
...
for t = 1:horizon
    ...
    demand = randi([dmin,dmax]);
    residualDemand = demand;
    
    for k = 1:length(onHand)
        if residualDemand <= onHand(k)
            onHand(k) = onHand(k) - residualDemand;
            residualDemand = 0;
            break
        else
            residualDemand = residualDemand - onHand(k);
            onHand(k) = 0;
        end
    end
    
    totalWaste = totalWaste + onHand(end);
    
    for k = length(onHand):-1:2
        onHand(k) = onHand(k-1);
    end
    onHand(1) = 0;
    
    onOrder = S - sum(onHand);
end
\end{lstlisting}

\paragraph{Checklist}
arrivi $\Rightarrow$ DES \\
tempo discreto $\Rightarrow$ time--driven \\
stato + decisione $\Rightarrow$ DP \\
scenari $\Rightarrow$ valore atteso
